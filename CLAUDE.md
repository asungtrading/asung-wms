...
- ⚠️⚠️ **작업 후에는 요약이 아니라 diff 실물을 제시한다 — 모델과 무관한 규칙이다.**
  보고 요약은 "무엇을 하려 했는가"를 말하고, diff 는 "무엇을 했는가"를 말한다. 둘은 자주 어긋난다.
  [실측 2026-08-18] 발주 축 재설계에서 요약은 정확했으나, **SR 폴백이 정말 없는지는 주석이 아니라
  코드로**, 카운터 위치는 **들여쓰기로만** 판정됐다. 요약만 보고 승인했다면 둘 다 놓쳤다.
  절차:
  1. `git --no-pager diff --stat` — 건드린 파일이 예상과 같은지 먼저
  2. `git --no-pager diff <파일>` — 범위가 크면 검토 지점만 잘라서 제시
     (⚠️ pager 에 걸리므로 `--no-pager` 또는 파일로 저장해 전달)
  3. 위험 지점은 **grep 근거를 함께** — 예: "폴백 없음"은 해당 심볼의 등장 줄을 전부 세어 보인다
  4. 커밋은 Caleb 이 diff 를 본 뒤에

- ⚠️⚠️ **`git push` 는 Claude 가 실행하지 않는다.** 커밋까지도 Caleb 확인 후.
...
- **여러 파일을 고쳤으면 정합 확인을 함께 보고한다.** 같은 사실이 설계 문서·스킬·코드 주석에
  ...
  [실측 2026-08-18] 「Apply 후 10분 자동 전환」 정정에서 지시된 3개 문서 외에 **2곳이 더** 남아 있었다
  (아침 세션 문서 · `inv-collect` 주석).
