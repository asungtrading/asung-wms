...
- 현재 여유: `asung-wms` **1017자(7자)** · `cin7-api` 840자(184자) — asung-wms 에 키워드를 더할 땐 반드시 같은 양을 덜어낼 것.
  ...
  (2026-08-04 갱신: `stock_short`·`픽리스트 인쇄` 추가 + `규칙 20~41` — 상계는
  **`재평가` 제거**(cin7-api description 이 이미 갖고 있는 중복어) + "반드시 먼저 읽으세요"→"먼저 읽으세요" ·
  "먼저 확인하세요"→"확인하세요" 압축. ⚠️ **여유가 7자다** — 다음에 키워드를 더하려면
  먼저 덜어낼 것을 정하고 시작할 것.)

- ⚠️⚠️ **다음 규칙(42)을 추가하면 한도에 닿는다 — 미리 읽을 경고 (2026-08-04)**
  규칙 번호 범위 표기 `규칙 20~41` → `규칙 20~42` 만으로 1자, 새 트리거 키워드까지 넣으면 즉시 초과다.
  **먼저 덜어낼 것을 정하고 시작할 것.** 압축 후보(순서대로):
  1. 규칙 번호 앞의 열거 압축 — `아키텍처·스키마·인증·배포·규칙 20~41` → `아키텍처·스키마·배포·규칙 20~42`
  2. **해소된 함정어 제거** — 그 함정이 재발하지 않으면 트리거 가치가 낮다.
     후보: `"Lines is invalid"`(2026-07-24 해소) · `"스캔 이어받기"`(규칙 22 정착) ·
     `"unconfirmed"`/`"writeChain"`/`"serverChecks"`(규칙 24 내부 구현명 — 사용자가 이 말로 부를 일이 드묾)
  3. `cin7-api` description 과 겹치는 단어 제거(그쪽이 먼저 뜬다)
  4. 그 다음이 일반어·영한 중복쌍
  ⚠️ 불변식 문구(위 항목)는 **마지막까지 유지.** 같은 내용이
