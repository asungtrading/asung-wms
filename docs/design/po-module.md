...
⑤ PO 본체          🔄 **설계 판단 확정(2026-09-15 저녁 · §11 · ⚠️ 표 정의는 다음 단계)** · 원장·원가 이식은 PO 뒤(§12) · 제품 생성 규칙 §11-k
```
...
빠지는 57   Service 53 · Non Inventory 4 — 전부 청구서 줄(아마존 프렙·드롭십·배송비·회계 조정·시스템 항목)
            ⬜ 별도 작업 — ⑤ PO 비용 라인이 참조할 대상이다. 제외는 맞되 담을 자리를 ~~⑤ 전에~~ [2026-09-15] ⑤ **표 설계 때 함께** 정한다(§7 · §11-f)
...
## 11. ⭐⭐ ⑤ PO 본체 — 설계 판단 (2026-09-15 저녁(집) · ⚠️ 판단만 · 표 정의는 다음 단계)

⚠️ 이 절은 **오늘 대화에서 나온 판단의 기록**이다. 표(DDL)도 화면도 없다. 표는 이 절을 입력으로 다음 세션에서 세운다.
...
⭐ **원본 번호 PO-12345 는 남지 않는다 — a 가 된다.** 껍데기 문서를 두지 않는다.
   ...
   📌 [검토 Claude 추가] 공급처 인보이스에는 PO-12345 로 적혀 온다 — 찾기는 접두어로 한다. 번호 형식(지금 Cin7 은 PO-01010 다섯 자리)은 표 설계 때.
...
⭐ 입력 단위를 고를 수 있다(Caleb: 「참 좋은 아이디어야」 · 오늘 채택)
   ...
   ⭐ **사람이 넣은 단위를 라인에 남긴다** — 인보이스와 대조할 때 「이 줄은 케이스로 넣은 것」이 보이면 검산이 된다.
      📌 [검토 Claude 제안 · 표 설계 때 판단] 그때의 환산 계수도 함께 남긴다 — pack_factor 가 나중에 바뀌어도 그날의 환산은 그대로여야 한다(§4-d 「그 순간에만 존재한 것」).
...
⬜ 표 설계 때 정할 것 (단가 · Caleb 실측 2026-09-15):
   ...
   ⬜ 기본값을 Fixed / Last 중에 고르는 옵션을 우리도 둘지는 나중에 정한다.
```
...
⭐ **비용은 스탁을 다 받은 뒤에 붙는 경우가 많다**(Caleb) ⇒ 입고가 끝나도 문서가 바로 안 닫힌다(11-b).
   ⚠️ 원가는 고치는 것이 아니라 **더한다** — `inv_layer_cost_add` 가 이미 그 자리다(§12-c).
⬜ Type=Service 53 · Non Inventory 4 를 담을 자리(§3-d·§7 미결)가 여기서 쓰인다 — **표 설계 때 함께 정한다**(§7 의 「⑤ 전에 정한다」를 「⑤ 표 설계 때」로 갱신).
```
...
⭐⭐ **저장은 인보이스 한 장 = 한 행** — 그 행이 PO 들을 가리킨다(저장만 다르다)
...
⬜ KRW 송금의 환차손익(§7)도 결제가 IMS 에 서면 여기서 다시 올라온다.
```
...
⬜ 사건을 실제로 어떻게 전달하는가(표 · 트리거 · EF)는 표 설계 때 정한다 — 오늘 미결.
```
