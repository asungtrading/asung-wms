...
### 6-b 길 셋 — `channel` · 유입 넷 — `source`

```
channel  warehouse · pos · counter          오더가 **가는 길**   ← 상태 전이의 모양을 정한다
source   manual · shopify · csv · pos       오더가 **들어온 경로** ← 전이와 무관 · 통계·연동의 축
```
⭐ **두 축을 가른다**(Caleb 판정 2026-09-22 · ⬜① 채택). Shopify 는 **유입 경로**이고 길은 `warehouse`(픽·팩 있음)를 탄다. 축을 섞으면 「Shopify 오더는 어느 길인가」를 물을 수 없다. 대가는 칸 하나. ⇒ 5-d 의 「⬜ Shopify 는 연동 때 `channel` 에 늘린다」는 **`source` 에 늘린다**로 바뀐다(5-d 본문은 고치지 않았다 · 6-i). 📌 `pos` 는 둘 다에 있다 — POS 화면에서 들어와(`source`) POS 길(`channel`)을 탄다.
...
```
CHECK     status 어휘 열 · channel 셋 · source 넷 · closed_reason              값만 막는다
...
| 5-d `channel` | 「창고 경유 · POS **둘** · ⬜ Shopify 는 연동 때」 | **셋** — `warehouse` · `pos` · `counter`(새 길 · 처음 이름 `direct` → 09-22 `counter`). Shopify 는 `channel` 이 아니라 **새 칸 `source`**(manual · shopify · csv · pos)에 담는다 — 칸 하나가 는다. 5-d 본문은 고치지 않았다 · CHECK 어휘라는 것은 그대로 | 6-b · Caleb 2026-09-22 ⬜① |
...
§3-c 여덟 중 이번에 ✅ 표시한 것 **둘**(픽 취소·WMS 롤백 · 수요 표). 5-h 의 셋과 합쳐 **다섯**이 닫혔다. 남은 것 **넷**(5-h 「남은 것 다섯」에서 롤백을 뺀 수): 가용 재고 계산/저장 · 일부 할당 상태의 창고 이동 · 브랜치 토글 자리 · 메일 템플릿·자동 발송.

### 6-j 이번에 정하지 않은 것 (⬜ 모음)

```
closed_reason 의 voided 가 어느 경우인지 · 그 밖의 이유                                          6-a · ④
so_out 사건의 모양 · WMS 사건(픽 시작·팩 완료·출하·롤백)을 IMS 가 받는 방법                    6-d · 6-e · ③ 사건과 원장 접점
