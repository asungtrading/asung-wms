...
--  ⚠️⚠️ **[정정 2026-09-09 실측] 그 검산은 성립하지 않는다.**
--  「레이어 잔량 = 원장 잔고 − 미처리 사건 합」이 맞을 것 같지만 **원리적으로 안 된다** —
--  미처리 사건으로 들어온 재고는 레이어가 없으므로 **그 SKU×창고의 판매가 엉뚱한 레이어(기초)를 대신 먹는다.**
--  수량은 그럴듯하게 남지만 어느 레이어에서 빠졌는지가 틀리고, 그 어긋남은 미처리 사건 합으로 환산되지 않는다.
--  [실측] real 축 잔차 **−11,449** · IN_TRANSIT **+1,247** · `short_events` 로 설명되는 몫은 1,087 뿐(부호도 반대다).
--  ⇒ ⭐ **잔량 대조가 의미를 갖는 것은 사건 9종을 다 처리한 뒤**다. 부분 처리 상태에서 차이를 맞추려는 시도는 하지 말 것.
--
