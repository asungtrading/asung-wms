...
-- ⚠️⚠️ [정정 2026-09-09 · Caleb 확인 · 주석만 고침 — 이 마이그레이션은 이미 적용됐다] **Cin7 도 FIFO 다.**
-- [실측 화면] 제품 마스터의 Costing method 가 FIFO 다(SKU 마다 설정).
-- ⇒ ⭐ 「방식이 다르니 당연하다」로 넘어갈 자리가 없다 — 금액이 벌어지면 원인을 규명해야 한다.
-- ⚠️ 그러나 완전히 같을 수는 없다. 우리 기초는 「8/20 평가액을 뭉친 한 덩어리」라 그 시점 Cin7 의
-- 레이어 구조를 이어받지 못했다. ⇒ 기초분이 소진될 때까지 구조적 차이가 남는다(재고의 96%가 기초분이므로 1년 이상).
-- ⬜ 미확인 — Cin7 FIFO 의 단위가 SKU 인지 SKU×창고인지. ⭐ 실무 정황은 SKU×창고 쪽이다 — 트랜스퍼 운송비가
-- Debit='_59_' 로 에드먼튼 재고에 녹으므로 창고별 원가가 갈려야 한다. ⇒ 대조 단계에서 창고별로 비교하면 답이 나온다
-- (에드먼튼이 크게 벗어나면 SKU 단위 · 맞으면 SKU×창고).
-- 📌 위 「bin 별 단가가 사실상 동일」(13,830칸 중 다른 것 13개 · 최악 1.09배)은 여전히 사실이다. 다만 그것은
-- 「가중평균이다」가 아니라 「bin 단위로 레이어를 가르지 않는다」는 뜻이다. 정본: ledger-design §원가 레이어 13번(미결).
--
