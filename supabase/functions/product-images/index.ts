...
//    non-200 페이지 없음). 하나라도 어긋나면 **한 행도 쓰지 않고** 진단만 반환.
//    분할 실행(이어받기)은 만들지 않는다 — 중간 상태를 저장하는 순간 그게 곧
//    부분 갱신 경로가 된다. 400초 예산(유료 플랜 문서 한도) 안에 넉넉히 끝난다.
...
//    절대 넣지 말 것(PUBLIC) — supabase secrets set + 대시보드 cron 등록에만.
...
const TIME_BUDGET_MS = 330_000;  // t0 기준. 초과 시 **쓰기 없이** 중단(aborted:"time") —
                                 ...
                                 //   EF wall-clock 한도는 유료 400초(문서) — 그 앞에서 우리가 먼저 끊는다.
...
// 실행 기록 1행 + 보존 정리(90일) — 둘 다 best-effort: 기록 실패가 응답을 막지 않는다.
...
Deno.serve(async (req) => {
  ...
  // 진단은 성공·실패 공통으로 이 객체에 쌓고, 응답과 run 기록(diag jsonb)에 같이 싣는다.
  const diag: Record<string, unknown> = {};
  let updated = 0;   // 밖에서도 보이게 — 배치 도중 예외 시 catch 가 "여기까지 썼다"를 기록한다
  try {
    ...
    if (aborted) {
      ...
      const noteSaved = await recordRun(startedAtIso, false, 0, errorNote, diag);
      return json({ ok: false, ...diag, note_saved: noteSaved });
    }
    ...
    const noteSaved = await recordRun(startedAtIso, true, updated, null, diag);
    return json({ ok: true, ...diag, note_saved: noteSaved });
  } catch (e) {
    // 배치 도중 예외 = 부분 갱신 가능 상태 — 각 행은 올바른 값이고 재실행이 수렴한다(멱등).
    // "이미지가 사라지는" 방향의 실패는 없다(빈 값을 쓰는 경로 자체가 없음).
    diag.updated = updated;
    diag.duration_ms = Date.now() - t0;
    const noteSaved = await recordRun(startedAtIso, false, updated, String(e).slice(0, 400), diag);
    return json({ ok: false, error: String(e), ...diag, note_saved: noteSaved }, 500);
  }
});
