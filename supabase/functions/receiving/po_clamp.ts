...
//   · ⚠️ SKU 합계 expected 0 = 공장 백오더(인보이스에 없는 오더 라인) — **클램프 제외, 받은 대로.**
//     0 으로 자르면 받은 물건이 Cin7 에 아예 안 들어간다(재고 누락) — 절대 금지 (2026-08-10 확정).
//   · ⚠️ expected_source='order'(구형 receipt · 인보이스 폴백)는 호출측이 clamp=false 로 부른다 —
//     오더 기준으로 자르면 정상 수량이 잘린다.
//
// ⚠️⚠️ 수식은 buildApplyPlan 트랜스퍼 절의 bin 이동 캡(Math.min budget 소진)과 쌍둥이지만
//    **expected 0 처리가 정반대다**: 트랜스퍼 = 전량 제외(물리적 강제 — 초과분이 Cin7 에 없어 쏘면 400) /
//    PO = 통과(정책 선택 — Cin7 PO stock received 는 초과 허용이 실측 사실). 한쪽을 고치면 반드시
//    반대쪽을 확인할 것. 공용 헬퍼 통합은 트랜스퍼 경로가 다음에 열릴 때 별건(2026-08-10 결정 —
//    트랜스퍼 병렬 배치가 같은 날 배포돼 그 경로는 동결).
//
...
export function applyPoInvoiceClamp(
  ...
  for (const p of planLines) {
    ...
    if (clamp) {
      ...
      if (Number(expectedBySku[k] || 0) > 0) {   // expected 0 = 공장 백오더 — 통과 (위 헤더 주석)
