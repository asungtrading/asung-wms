...
② **파일을 만들었다는 보고를 믿지 마라**
   ...
   ⇒ 그 뒤 「원본 바이트를 스크립트로 옮기고 diff 출력을 붙이는」 방식으로 바꿔 두 차수를 무사히 넘겼다
...
⚠️⚠️ **그 함수의 마지막 정의가 어디인지 먼저 찾는다**
   ...
   [실사고] po_invoice_create 지시서를 옛 파일 기준으로 썼는데 실물은 뒤 파일이 덮어쓴 판이었다.
   그대로 갔으면 크레딧 자동 채번이 통째로 사라졌다 — 검토가 잡았다
...
⚠️ 적용 뒤에는 **실물로 확인한다** — 함수 개수·정책 수·반환 모양
   [실사고] SQL 도구의 「Success」를 psql 결과로 읽어, 적용하지 않은 마이그레이션을 적용한 것으로 알고
   한참 진행했다. 화면에서 메뉴 항목이 사라진 뒤에야 드러났다
```
...
⚠️⚠️ **SQL 도구는 문장마다 연결이 끊긴다** [2026-09-17 실측]
   ...
   (permission denied · pg_temp 없음 · 막혀야 할 것이 통과)
...
⚠️⚠️ **뼈대로 삼은 화면에서 진입점까지 그대로 가져온다**
   [실사고] 공통 함수 호출 모양을 기억으로 썼다(`imsAuth(...)` ← 실물은 `imsAuth.start({…}, cb)`).
   콜백이 한 번도 안 돌아 화면이 통째로 죽었다(헤더·메뉴·목록 전부)
...
```
대화 Claude 가 파일을 만들면 Caleb 이 내려받아 옮긴다
...
한도    ⚠️ 본문 **14,000바이트** · description **1024자** — pre-commit 이 둘 다 검사하고 여유를 출력한다
