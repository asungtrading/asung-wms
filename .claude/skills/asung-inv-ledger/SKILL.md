...
· `ledger_collected_at`·`ledger_lag_source` = 원장 값의 신선도(⚠️ `cost` 제외 · 재고 축 여섯 — ⚠️ [실측 09-11] `cost_transfer` 는 **제외되지 않아** cron 미등록인 채로 매일 `lag_source` 로 뜬다. 그 값으로 판정하지 말고 ⑤ 를 볼 것)
