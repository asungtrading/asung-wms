...
| | 적용 | Remote 빈 줄 |
|---|---|---|
| 운영 `asung-WMS` | 53 | **2** — `20260908195949` · `20260908202555` |
| 테스트 `Asung-IMS` | 55 | 0 |

⭐ **의도한 차이 (운영에 일부러 안 올린 것)**
| 마이그레이션 | 무엇 | 언제 운영으로 |
|---|---|---|
| `20260908195949` | 원가 레이어 그릇 (`inv_layer` 3표 + 뷰) | 사건 적재까지 검증 후 |
| `20260908202555` | 기초 적재 RPC `inv_layer_seed_baseline` | 위와 함께 |

⚠️ **이 표에 없는 빈 줄이 나오면 그것이 신호다.** 표를 갱신하지 않으면
「차이 2개」가 「차이 5개」가 되어도 괜찮은 줄 알게 된다 —
운영에 안 올리기로 한 것이 생기면 **여기에 줄을 추가**한다.
