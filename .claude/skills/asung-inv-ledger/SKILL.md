...
- ⚠️ **커서 교착 중 — `TR-03975`·`03976`** (2026-08-21~ · [실측 2026-08-25] 둘 다 여전히
  ...
  · 도착 시 leg 1·2 는 since 필터에 걸려 **leg 3·4 만 기표**(IN_TRANSIT 음수 잔존) — 📌 **대조
    무영향의 「흉터」이지 위험이 아니다**(Cin7 도 운송 중을 ON HAND 에서 뺀다 · 음수 행은
    짝이 없어 `explained`). ~~「도착 전에 손봐야 한다」~~ 는 2026-08-25 철회.
...
1. ⚠️ 스냅샷 급감 검사 (§4-c) — **여전히 미해결·최우선**.
   ⚠️ [2026-08-25 확인] `inv_snapshot` 으로는 구현 불가다: `count(*)` 는 회차값이 아니라 **누적
   합집합**이고(재촬영 시 첫 값 + 신규 조합), `min(taken_at)` 은 재촬영을 안 보여주며,
   `list_total`·페이지 수·소요 시간은 **아예 저장되지 않는다**. ⇒ **회차 로그 테이블 신설이 선행 조건.**
   ⚠️ 그리고 판정 축은 **행 수가 아니다** — [실측] 불완전 회차 −42 vs 정상 5일치 −25 로 구별 불가.
   가른 신호는 `list_total` 22,079→21,877 · 페이지 23→22 · 소요 43초→1분44초였다.
2. ⬜ `inv-sku-types` cron 등록 (등록 후 jobid 를 `ops/cron.sql` 에 반영)
3. 매일 아침 3줄 — 스냅샷 · 대조 unknown · `inv_missing_lines` 미해결
4. 30일 무결 후 하드 플립
