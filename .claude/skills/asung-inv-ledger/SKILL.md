...
| **D** (08-31) | ⬜ **②-a 비종결 홀드 + 캡 → 뒤쪽 문서 영구 누락** | **없음** |
...
⬜ **결함 D 처방 후보**(다음 세션 최우선):
· (C) 「홀드 목록」과 「커서 바닥」 분리 — 비종결 문서를 따로 추적하고 커서는 전진
· (D) **이미 원장에 있는 문서는 상세를 부르지 않는다** ⇒ 캡 여유가 크게 는다
· ❌ 커서를 비종결 문서 뒤로 미는 것 — **도착 다리를 영영 못 받아 실재고가 틀어진다**
...
2. 매일 아침 4줄 — 스냅샷 · 대조 unknown · `inv_missing_lines` 미해결 · 회차 로그
   ...
   ⚠️⚠️ **아침 점검에 「수집이 밀렸나」 축이 없다** (2026-08-31 확인). 결함 C·D 둘 다 대조가
   잡았지만 **하루 늦었다.** 그리고 **SQL 만으로는 부족하다** — `hold_capped`·`detail_capped`·
   `cursor_stalled_alert` 는 **EF 응답에만 있고 DB 에 안 남는다.**
   ⬜ **EF 회차 통계를 테이블에 남기는 설계 필요**(`inv_snapshot_runs` 가 스냅샷에 한 역할과 동형).
   📌 그때까지는 `inv_sync_state` 의 커서·`last_run_at` 을 눈으로 보는 것이 최선이다 —
   ②-b 는 커서가 타임스탬프라 「지금에서 얼마나 뒤처졌나」가 바로 보인다.
3. ⚠️⚠️ **「30일」은 근거가 없는 숫자다** (2026-08-28 재검토). 재려는 것은 「시간이 흘렀다」가
   아니라 **「일어날 만한 일이 다 일어났다」**다 ⇒ **날짜가 아니라 사건 목록으로 센다.**
...
| ③ 새 결함 빈도가 떨어졌다 | ⚠️ 아직 **사흘에 하나꼴**(8일간 3건) |

📌 **③이 날짜를 요구하는 유일한 이유**다. 그리고 결함 셋(`FINAL-SALE`·`TR-04175`·`PO-01133`)은
**새로운 사건 종류가 아니라 드문 조합**이었다 ⇒ 시간이 필요한 이유는 새 종류를 기다리는 게
아니라 **드문 조합이 나올 확률을 쌓는 것**이다. 그 조합은 **실무가 만든다.**
