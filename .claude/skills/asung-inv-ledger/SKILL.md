...
5. ⚠️⚠️ **미출고 31건 · 6,105개 — `ship_date` 경계 폭탄** (09-04 신규 · §함정 표
   `ShipmentDate` 행이 정본). 8/20 이전에 준비되고 아직 안 나간 오더들이며, **`ship_date` 가
   옛 날짜로 남아 있는 것은 나갈 때 원장이 못 받는다.** [실물] `SO-14734` 가 9/1 에
   그렇게 터졌고 09-04 에 상쇄했다. ⚠️ **위험 판정은 오더 날짜가 아니라 `ship_date`** 이고
   그 값은 Fulfilment 안에 있어 오더 목록에서 안 보인다 — ⬜ **모집단을 그 축으로 다시
   세는 것이 첫 할 일**이다. ⬜ 처방 후보 셋(실무 규칙 · 감시 목록 · 사건 날짜 축 변경)
6. ⬜ **커서 근본 해결** — 홀드 목록과 커서 바닥 분리
7. ⭐ **WMS 재고 마스터 화면** — 원장 준비 완료
