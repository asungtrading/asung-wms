...
`id`(PK identity), `task_kind`(pick/pack/wave CHECK), `task_id`(다형 참조 — FK 없음, rollback delete 후에도 사실 기록 보존), `worker`(⚠️ **2026-08-24 의미 재정의: "멈춤 시점의 소유자"** — manual 이면 Hold 누른 사람과 동일 · auto 면 시스템이 그 사람 몫을 대신 멈춘 것으로 버튼은 안 눌렀다. 구분자는 source), `held_at`(default now — ⚠️ auto 는 명시값 「마지막 활동+10분」을 넣는다: 과거 시각이 정상), `resumed_at`(null=열림), `resumed_by`(재개자 — 남의 held 재개 허용이라 worker 와 다를 수 있다), `source`(manual/auto CHECK). RLS auth_all + anon revoke. 인덱스 `(task_kind, task_id)`(부분 유니크 금지 — on_conflict 규칙). **wave 는 wave 행 1건**(멤버별 N행 금지 — Hold·재개가 전원 동시 원자라 중복이고 「Hold 횟수」를 N배 왜곡. 멤버 단위는 `wave_id` 조인으로 복원). 쓰기 = Hold RPC 2종(마지막 문장 insert — 예외 시 전체 롤백에 포함) / 닫기 = 재개 프론트 3곳(startBatch·startWave·resumePack) fire-and-forget `.is("resumed_at",null)` UPDATE. ⚠️ 열린 채 끝나는 4경로(rollback pick reset/pack undo·오더 rollback·void·unwave)는 의도적으로 닫지 않는다 — 계산 규칙(「태스크가 지금도 pending+held_by」일 때만 진행 중 취급)과 health `hold_leak` 이 방어. Stats 계산 변경은 다음 단계.
