...
- ⚠️⚠️ **2026-08-13 보강 (규칙 29 의 또 한 사례 — 문서 말고 실물 DB): 위 2026-08-04 목록에 없던 컬럼 5개가 실물에 존재한다** (`select *` 실측). ① **`pick_task_id`·`pack_task_id`**(bigint — `20260805000000_completed_by_pack_link.sql`): 이 discrepancy 가 어느 단계·어느 작업의 산물인지 링크. ⚠️ **FK 를 의도적으로 걸지 않았다**(그 마이그레이션 명문: 롤백이 task 행을 delete 하므로 FK 면 CASCADE 든 SET NULL 이든 **증거가 사라진다** — 링크는 id 값으로만) → **PostgREST 임베드 불가, `.in` 일괄 조회가 유일한 경로**(admin `_srcGone` 배지·short_pick 픽커 표시가 이 패턴). ② **`voided_at`·`voided_by`·`voided_reason`**(`20260806120000_rollback_archive_disc_void.sql` — 롤백 무효화: 삭제가 아니라 태그. 규칙 41 이 `voided_at` 을 이미 언급하는데 이 컬럼 목록에는 빠져 있었다).
...
테이블 아님(함수). `returns table(sort,check_key,category,title,hint,fail_count,sample jsonb)`, `security definer`+`set search_path=public`, authenticated grant, 읽기 전용, `create or replace`. admin Health 탭이 `sb.rpc("wms_health_check")`로 호출. 검사 13개(각 CTE: bad_math/factor_drift/split_bad/short_no_disc/pick_over/progress_leak/dup_sale/finalize_recon/orphan_pick/orphan_pack/wave_state + image_sync_stale 120(`20260814030000`) + hold_leak 130(`20260824192416`))+info 1(last_import). fail_count=0이 건강, sample=위반 최대 8행. ⚠️ orphan_pack은 pack의 짝 pick(`pick_task_id`)이 completed 아닐 때만(배치 병렬 정상케이스 오탐 방지). 규칙 19. ⚠️ **최신 정의는 baseline 이 아니라 마지막 재정의 마이그레이션이다** — 검사를 더할 땐 재정의 전수를 grep 으로 확인하고(자르지 말 것) 그 전문을 기반으로 할 것(2026-08-24 에 baseline 을 기반으로 삼아 image_sync_stale 을 지울 뻔했다).

## wms_task_holds — Hold 구간 이력 (2026-08-24, `20260824192416_task_holds.sql`)
`id`(PK identity), `task_kind`(pick/pack/wave/**receipt**(2026-08-25 `20260825203457`) CHECK), `task_id`(다형 참조 — FK 없음, rollback delete 후에도 사실 기록 보존), `worker`(⚠️ **2026-08-24 의미 재정의: "멈춤 시점의 소유자"** — manual 이면 Hold 누른 사람과 동일 · auto 면 시스템이 그 사람 몫을 대신 멈춘 것 · ⚠️ **receipt 는 「누른 사람」일 뿐 소유자가 아니다** — 문서 공유(동시 다인)라 구간의 의미는 「문서의 멈춤」), `held_at`(default now — ⚠️ auto 는 명시값 「마지막 활동+10분」: 과거 시각이 정상), `resumed_at`(null=열림), `resumed_by`(재개자 — worker 와 다를 수 있다), `source`(manual/auto/**partial** CHECK — partial = 리시빙 선적 대기). RLS auth_all + anon revoke. 인덱스 `(task_kind, task_id)`(부분 유니크 금지 — on_conflict 규칙). **wave 는 wave 행 1건**(멤버별 N행 금지 — 멤버 단위는 `wave_id` 조인으로 복원). 쓰기 = Hold RPC 2종 + `wms_pause_receipt`(전부 플립+insert 한 트랜잭션) / 닫기 = **RPC `wms_resume_hold`(20260825183827 — resumed_at 서버 시계·resumed_by 서버 유도. ~~프론트 UPDATE~~ 는 태블릿 시계가 11행 음수를 만들어 폐기)** — 호출 = 픽·팩 재개 3곳 + 리시빙 3곳(receiver 재개 · admin Reopen · 완료 시[A Hold→B 완료 엣지]). ⚠️ 열린 채 끝나는 admin 4경로(rollback·void·unwave)는 의도적으로 닫지 않는다 — 계산 규칙(태스크가 지금도 pending+held_by / receipt 는 held·partial 일 때만 진행 중 취급)과 health `hold_leak`(receipt 판 포함)이 방어. Stats = 픽·팩 C 병기(2026-08-25) + Receiving 표 「Avg receive − holds」(같은 날) — 규칙 37 계열이 정본.
...
id BIGINT PK · po_number TEXT NOT NULL (PO-xxxxx / TR-xxxxx) · cin7_purchase_id TEXT (PO GUID/TR TaskID — 안정 키, 열린 receipt dedup) · supplier_name TEXT · warehouse TEXT NOT NULL default 'toronto' CHECK(toronto/edmonton) · status TEXT CHECK(in_progress/held/partial/completed) · **source_type TEXT default 'po' CHECK(po/transfer)** · received_by TEXT (⚠️ **"receipt 를 처음 만든 사람"** — 받은 사람이 아니다 · 이후 갱신 안 됨 · 라인별 작업자는 2026-08-17 wms_receipt_lines.last_received_by) · note TEXT · **applied_at TIMESTAMPTZ / applied_by TEXT / apply_note TEXT** (Apply to Cin7 성공 기록 = 이중 반영 방지 키) · created_at/updated_at/completed_at.
...
id BIGINT PK · receipt_id FK cascade · cin7_po_line_id TEXT · order_sku/base_sku/product_name · expected_base NUMERIC (PO 예상 낱개, 오프-PO=0) · received_base NUMERIC (받은 누적 낱개) · exported_base NUMERIC (**2026-07-28 부터 트랜스퍼 bin 이동 체크포인트** — 이미 Cin7 에 옮긴 낱개. 재Apply 때 이 라인을 건너뛰는 유일한 근거이므로 ⚠️ **사람이 수동 UPDATE 하지 말 것**(규칙 30-4). PO 경로는 아직 미사용) · putaway_bin TEXT (자동확정/수동 bin) · zone TEXT (가이드 동선 정렬) · putaway_done BOOL · is_off_po BOOL · needs_approval BOOL (승인 전 풋어웨이·Apply 차단) · approved_by/approved_at · verification_method(scanned/manual) · status(pending/received) · **last_received_by TEXT / last_received_at TIMESTAMPTZ (2026-08-17 · `20260817000000_receipt_lines_last_received_by.sql`)** — **이 라인을 마지막으로 만진 작업자**(라인당 1명 · receipt 참여자 전원은 distinct 로 드러난다).
인덱스: idx_receipt_lines_receipt, idx_receipt_lines_base, idx_receipt_lines_export(부분 — 구 CSV 용, 현재 미사용).
⚠️ **last_received_by/at**: NULL = 컬럼 도입(2026-08-17) 전 라인 — **백필 안 함**(과거 작업자는 알 수 없다 · received_by 복사 금지 — 그게 바로 틀린 값이다). 표시는 **라인 값 우선 → 전부 NULL 이면 `wms_receipts.received_by` 폴백**(화면에서 회색 `~이름 (started by)` 로 구분 — 폴백은 "시작한 사람"이라 정확하지 않다). ⚠️ **`approved_by`(off-PO 승인자)와 다른 것 — 재사용 금지.** `wms_receipts.received_by` 는 "receipt 를 처음 만든 사람" 의미로 유지된다. 기록 주체는 receiver `patchFor` — 실작업만 찍는다(`putaway_auto` 자동배정·`startPo` 초기 적재 제외 — 규칙 24 라인별 작업자 항목).
...
- ⚠️ **[실사고 2026-09-01] manager 분할 인쇄에서 두 필드 누락 — 08-30 검증이 못 잡은 이유: 호출부 4곳에 `terms/shipAddress` 파라미터를 넣는 것만 확인했고, 그 파라미터가 읽는 객체가 어디서 오는지는 안 봤다.** manager `pickOrd`(:519)만 selected 에서 필요 필드를 골라 담는 새 객체라 두 필드가 undefined 였다(픽커·팩커 재인쇄는 `select("*")` 재조회 객체 그대로, printWaveAll 도 orders 원본 행 그대로 — 셋 다 무사). 수정 = pickOrd 에 `terms`/`ship_address` 추가. 교훈: **파라미터를 추가하면 호출부가 "넘기는가"만이 아니라 "넘기는 값의 출처 객체에 그 필드가 있는가"까지 볼 것** — 필드를 골라 담는(pick) 객체가 있으면 그곳이 첫 용의자다.
