...
- ❌ **child-location 이름 매칭 경로 제거.** ~~child 의 `Name` 은 bin 이름이 아니다(실측 예 `"071164313169"` — 바코드류) → 처음부터 매칭될 수 없는 죽은 폴백이었다.~~ → ⚠️⚠️ **[정정 2026-09-11] child 행의 `Name` 은 bin 이름이 맞다**(전량 2,678행 실측 · 2,675/2,675 일치 · 바코드 0). ⭐ **폴백이 죽은 이유는 이름이 아니라 `Limit 500` 잘림**(에드먼튼 child 0행) — 근거가 틀렸던 것이고 처방은 그대로다: `Bins[]` 경로가 여전히 정답이며 **되살리지 말 것**(`binGuid` 무변경). 상세 `cin7-api/references/stock-write.md` 5절.
