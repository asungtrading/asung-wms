...
- **`wms_health_check()` DB 함수**(`wms_healthcheck.sql`, `security definer`+authenticated grant, 읽기 전용, `create or replace`라 재실행 안전). admin.html **Health 탭**이 `sb.rpc("wms_health_check")`로 호출, 검사당 카드 하나. **0행=건강**(fail_count>0이면 깨진 것, sample은 위반 최대 8행 jsonb). ⚠️ **자동 실행은 없다 — 사람이 탭을 눌러야 돈다**(2026-08-13 백로그 「신규 기능」 Health 자동 실행 항목 · 우선순위 1위).
...
📌 **우선순위 (2026-08-13 저녁 확정)**: ~~EF 게이트~~(✅ 완료 — 「보안」) → **Health 12검사 자동 실행**(「신규 기능」) → Cin7 void 감지 → packer donePick 근본 수정 → "잘린 모수" 전수조사 → 재고 원장 shadow mode. 근거: 앞의 것들은 **"지금 틀리거나 뚫려 있는 것"** 이고 원장은 새로 짓는 것 — 기반이 새는 상태에서 층을 올리면 나중에 둘 다 의심하게 된다.
...
| # | 위치 | 상태 |
|---|---|---|
| 1 | fulfillment 완료 팩(보드 원천) | ✅ 응급 수정(내림차순+LIMIT — 998/1000 에서 2건 전) |
...
3호(packer donePick·allPack)는 2026-08-12 응급 수정 완료. 근본 수정(!inner 안 닫힌 오더만)은 「동시 작업 원자화」 위 packer 항목.
...
- **리시빙 동시 작업 미해결분 — 규칙 27 이 전체 목록** (위 「리시빙 Apply」·「동시 작업 원자화」에 안 담긴 것): **R3 `wms_receipts.cin7_purchase_id` 유니크**(중복 0건 확인, 분할 입고는 새 PO 라 걸어도 안전 — 새 마이그레이션) · ~~R4 Apply 최종 PATCH 에 `applied_at is null`~~(✅ 2026-08-06 in-flight 잠금으로 PO·트랜스퍼 해소 — 규칙 27 R4) · R5 Complete↔Apply 창 · RLS 창고 스코프 · ~~EF 호출자/perms 서버 검증~~(✅ 2026-08-13 `_shared/authgate.ts` — 백로그 「보안」 · 잔여 applied_by `&by=` 서버 유도도 그쪽).
