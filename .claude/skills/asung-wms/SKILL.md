...
- **완료 라인 저장은 묶음 upsert (2026-08-06 · ⚠️ 현장 미검증)**: picker `finish`·packer `doneBtn` 의 라인 최종 저장이 라인당 순차 PATCH(60줄=60요청≈9.5초 실측, 중간에 끊기면 반쪽 상태)에서 **단일 upsert POST**(`on_conflict=id` — PK 전체 유니크, 규칙 29 충족)로 바뀌었다. 한 요청 = 단일 `INSERT..ON CONFLICT DO UPDATE` 문 = **전부 아니면 전무**. ⚠️ **전제 = 라인 id GENERATED BY DEFAULT**(`20260806130000_pick_pack_line_id_by_default.sql` — GENERATED ALWAYS 면 PostgREST 가 명시 id 를 **428C9 로 거부**, 2026-08-06 원격 실측. **ALWAYS 로 되돌리면 완료 전면 중단.** 배포 순서 SQL 먼저 — 규칙 23). 컬럼 유실 없음: ON CONFLICT UPDATE 는 **페이로드 컬럼만 SET**(안 보낸 assigned_base/expected_base 등 불변), 행이 삭제된 라인(롤백 경합)은 INSERT 분기 NOT NULL 위반으로 **배치 전체 실패(fail-closed — 유령 행 부활 없음)**. ⚠️ `verification_method` 는 `||null` 정규화 필수(undefined 는 JSON 키 탈락 → 행마다 키가 달라져 PostgREST 거부). 재시도는 절대값 재전송이라 **멱등** — 실패 시 `resyncAfterFail` 이 서버 재조회로 "이미 저장됨(응답 유실)/미저장" 을 구분 안내하고 **로컬 수량은 버리지 않는다**(규칙 28 로컬 flush 금지와 같은 방향). 쓰기 구간은 `scanBusy` 로 스캔 차단. 같은 커밋: packer discrepancy insert 3곳 `mustRows`(로컬 복사 — 공용 모듈 아님) + `checkOrderReady`(⚠️ **0행은 멱등 가드의 정상 no-op — error 만 판정**, 실패는 완료 성공 후이므로 바깥 catch 로 안 보내고 "Completed — order status update failed" 별도 안내) + packer `saveLine` 확인 추가(규칙 20). **남는 틈(라인 배치→discrepancy 문장들→플립, 비멱등 insert 재시도 중복)은 규칙 28 한계 절** — 8단계(RPC) 영역. **Hold 도 동일 패턴(별도 커밋)**: picker·packer Hold 의 라인 루프도 같은 묶음 upsert — 이 루프 2곳은 **결과 무확인이었다**(기준선 감사 정독 추가 발견 +2), packer Hold 의 태스크 플립 error 확인도 picker 와 대칭 교정. ⚠️ 리시빙 라인 저장은 대상 아님(규칙 20·24 — 여러 명이 나눠 받는 구조라 라인 단위가 맞다, 묶으면 동료 작업이 사라진다).
...
- **⚠️ 발견 기록**: packer 완료 UPDATE 는 CAS 이전까지 **error 를 아예 확인하지 않았다** — 완료 실패가 조용히 무시되고 화면은 성공 흐름을 탔다(CAS 와 함께 throw 로 교정). pack **라인 최종 저장 루프와 discrepancy insert 들도 같은 패턴으로 error 미확인**(picker 는 확인함) — ~~미수정, 별건~~ → **✅ 같은 날 후속(완료 라인 묶음 upsert — 규칙 9)으로 해소**: 라인 최종 저장 = 단일 upsert(원자), discrepancy insert 3곳 `mustRows`, `checkOrderReady` error 판정(0행은 멱등 가드 정상), packer `saveLine` 확인 추가.
- **남는 한계**: ~~finish 단계의 라인 최종 저장·discrepancy insert 는 여전히 무조건~~(→ 2026-08-06 묶음 upsert 후속으로 해소 — 위 발견 기록) · 남는 것 = **라인 배치 → discrepancy 문장들 → 완료 플립 사이의 틈**: discrepancy insert 는 비멱등이라 중간 실패 후 재시도에서 **중복 행 가능**(`uq_disc_receipt_sku` 는 receipt_id NULL 이라 픽·팩 행엔 안 걸린다) — 세 쓰기를 한 트랜잭션으로 묶는 것은 8단계(RPC) · `saveLine` 라인 단위 CAS 는 R1 계열 백로그 · wave 멤버 UPDATE 와 wave 행 UPDATE 사이의 밀리초 창은 남는다(조용한 덮어쓰기 → 시끄러운 실패로 바뀐 것) · claim_seq 원자화는 여전히 후속.
...
12. ~~**매니저 2명 동시 Apply (규칙 27 R4 · 규칙 35)**~~ → **✅ 2026-08-06 해소 — PO·트랜스퍼 공통** in-flight 잠금(`apply_lock_at/by`, 규칙 21). ⚠️ 현장 미검증. 트랜스퍼는 duplicate 400 방어가 없어 동시 실행이면 진짜 이중 이동이었다 — 같은 날 확장의 이유.
...
14. ⚠️ **초과 클램프 — 정책은 확정, 선행조건이 남았다** (2026-08-05 사용자 결정). **정책**: 부족은 지금처럼 그대로 두고(받은 만큼만 반영), **초과는 인보이스 수량까지 클램프**하며 **초과분은 매니저가 Cin7 에서 수동 조정**한다(discrepancy 큐 — 규칙 20 차이 처리와 같은 구조, 자동 조정 없음). **bin 감산은 마지막 bin 부터**(먼저 놓은 자리를 흔들지 않는다). ⚠️⚠️ **선행조건 4개 — 하나라도 안 끝났으면 시작하지 말 것**: ①**Apply 1회 성공 확인**(인보이스 기준 expected 로 실제 반영이 되는 것을 본 뒤 — 「검증 대기」 3번) ②**bin 이동 캡과의 관계 정리**(지금 캡이 이미 `min(received, expected)` 다 — 클램프를 또 넣으면 **두 곳이 같은 판정을 하게 된다.** 규칙 30-2 배너 중복 계산과 같은 실수) ③**`exported_base` 에 클램프된 값을 넣을 것**(체크포인트가 "실제로 문서에 실은 양" 이어야 재Apply 가 맞는다 — 규칙 21 PO 이식 절) ④**expected 0 라인 처리 결정**(인보이스에 없는 오더 라인 = 공장 백오더. 클램프하면 **전량 0** 이 되어 받은 물건이 Cin7 에 안 들어간다 → 규칙 20 의 "expected 0 라인은 buildApplyPlan 이 rb≤0 만 건너뛴다" 와 정면충돌. **여기서 정책을 한 번 더 정해야 한다.**)
15. ~~**Invoice First 게이트 fail-closed 전환 판단**~~ → **✅ 2026-08-06 전환 완료** (규칙 21 정정 항목 참조). 전환 조건이던 관찰은 폴백 배포(v30)+PO-01076 Apply 성공으로 충족. ⚠️ 현장 미검증 — 다음 실 Apply(AUTHORISED 통과 회귀)가 검증 1순위.
16. **Apply 병렬화 / bin 배치 분리** — 회차 시간을 줄일 후보(같은 SKU 가 여러 그룹에 있으면 동시 이동 충돌 → **같은 SKU 그룹 분리 필요**). ⚠️ **측정 후 판단** — v3 완주 보장으로 시급성이 내려갔다. TR 분할은 대안이 아니다(규칙 39 — 그룹 수가 오히려 는다).
...
### 리포트·통계
- ⚠️ **wave 모드 리포트 오더 귀속 버그 (규칙 14)** — **증상**: wave 는 여러 오더의 라인을 한 배치에 섞는데, `wrong_location`·`barcode_mismatch` insert 가 `order_id: task.order_id`(= wave 배치의 대표 오더 하나)로 **고정**돼 있다. 그래서 **B 오더 라인에서 누른 리포트가 A 오더에 붙는다.** 매니저는 admin Reports 에서 A 오더를 열어 그 SKU 를 찾지 못하고(A 에 없는 라인이다) 리포트를 **오작동으로 여겨 무시하게 된다** — 조용히 신뢰가 깎이는 종류다. **수정**: 같은 파일의 `image_mismatch` 가 이미 옳은 형태다(`wave ? l._orderId : task.order_id` — 라인별 귀속, `finish()` 의 discrepancy 와 같은 규칙 · 규칙 18). 두 kind 를 같은 식으로 바꾸면 된다. **범위**: picker(`wrong_location`·`barcode_mismatch`) + packer(`barcode_mismatch`). ⚠️ **이미 쌓인 잘못된 행은 소급 정정 불가**(어느 오더였는지 복원할 정보가 행에 없다) — 고친 시점부터만 옳다. **우선순위를 낮추지 말 것.**
- **공용 리포트 모듈 `wms-reports.js` 추출** (2026-08-05 후보) — ⚑ 리포트 로직이 지금 **4곳에 복제**돼 있다: picker(3 kind)·packer(2)·receiver(3, 2026-08-05 신규)·admin(표시). receiver 를 만들 때 **의도적으로 복제**했다(사용자 결정 — 검증된 픽·팩 코드를 건드리지 않기 위해). 이제 4번째가 생겼으니 `wms-picklist.js`·`wms-packing.js`·`wms-confirm-modal.js` 와 같은 **단일 출처** 패턴으로 뺄 시점이다. ⚠️ **위 wave 귀속 버그 수정과 반드시 묶을 것** — 따로 하면 같은 파일을 두 번 건드리고, 추출만 먼저 하면 **버그를 공용 모듈에 그대로 이식**한다. ⚠️ 추출은 픽·팩의 검증된 경로를 건드리므로 **동작 동일성 diff 를 먼저 증명**할 것(`_shared/cin7.ts` 추출 때와 같은 절차).
...
- ⚠️⚠️ **픽·팩 완료 라인 묶음 upsert (2026-08-06 배포 · 규칙 9)** — ⬜ 전부 현장 미검증. **① 컬럼 유실 diff 가 최우선**(완료 직전/직후 `select * from wms_pack_task_lines where pack_task_id=X order by id` 전체 덤프 비교 — 바뀌어야 할 컬럼(verified_base·status·verification_method·verified_at) 외 **전부 동일**이어야 통과. pick 테이블 동일 절차) ② 원자성 실측(로컬 `db reset` 환경에서 불량 status 1행 섞은 배치 → 400 + **0행 변경**) ③ 정상 완료 회귀(pack / pick 단일 / wave) ④ 대형 오더(60줄) 소요시간(9.5초 → <1초 기대) ⑤ 전 라인 stock_short 선언 완료 ⑥ checkOrderReady 실패 시 "Completed — order status update failed" 문구 + heartbeat 미재개 ⑦ Hold 회귀(픽·팩 각각 — 진행 보존 + 재개, 별도 커밋 분).
- ⚠️⚠️ **2026-08-05 배포 4건 — 전부 현장 미검증** (커밋 `b426275`·`d64ebff`·`70cf91d`·`9c97618`).
  ...
  3. ⚠️⚠️ **인보이스 기준 기대치 (`70cf91d` · 규칙 20 개정)** — ✅ 확인됨: **Simple PO 경로**.
     ⬜ **미확인 — 이게 가장 중요하다**: **Advanced 경로**(`invoiceBlock()` 의 배열[0] 분기 —
     PO-01068 이 Advanced 인데 이미 Apply 돼 재사용 불가) · **첫 Apply**(expected 가 인보이스로
     바뀐 receipt 를 실제로 Cin7 에 반영하는 것. bin 이동 캡이 `min(received, expected)` 라
     **expected 의 출처가 바뀌면 캡도 바뀐다**) · `expected_source='invoice'` 가 실제로 찍히는지 ·
     인보이스 폴백 경고 토스트가 실물에서 보이는지. **아직 안 받은 PO 로 검증할 것.**
...
- ⚠️ **스크롤 튕김 — 판별 미실시 (2026-08-05 후보)**. 작업 중 화면이 목록 맨 위로 튀는 현상. **원인 후보 2개이고 아직 어느 쪽인지 안 갈랐다**: ①**`pageshow` 리로드** — 태블릿에서 돌아올 때 브라우저가 페이지를 재실행해 스크롤이 초기화 ②**규칙 28 프리즈** — 소유권 확인이 걸려 화면이 리로드 모달 경로를 탄 것. **판별 먼저**: 프리즈 모달이 떴는지(②의 지문 — 모달 없이 튀면 ①)와 `visibilitychange`/`focus` 로그를 볼 것. ⚠️ **원인을 정하기 전에 수정하지 말 것** — ②라면 튕김은 **의도된 동작**이고(스테일 화면을 계속 쓰게 두는 것이 더 위험) 고치면 규칙 28 가드를 깬다. ①이면 **후보 수정 = URL 에 배치 id 를 남겨 복원**(`?batch=` — ⚠️ 상태를 localStorage 에 두지 말 것, 규칙 5).
