...
|---|---|---|---|
| 애플리케이션 트리거 | 0 | 0 | **0** |
| 트랜잭션 안 다중행 쓰기 | 0곳 | 4곳 | **4곳** |
| 자동 테스트 | 0 | ~38케이스 | **~47케이스**(+`po_clamp` 9) |
| DB 제약(unique·check) | 5 | 8 | 9 |
| 쓰기 무확인 | 26 | 5 | 5 (미감사) |
| 되돌릴 수 없는 쓰기 잠금 | 0/2 | 2/2 | 2/2 |
| 조사 없이 구현 | 0 | 0 | **0** |
| Cin7 하루 콜 | — | ~26,000 | **~12,000** |
| **세션당 조용한 결함 발견** | — | — | **3** ← 신규 |

⚠️ **트리거 0과 트랜잭션 4가 3일째 그대로다** — 그 벽 뒤에서 08-11 의 결함 3건이 전부 나왔다.
...
12. ~~**매니저 2명 동시 Apply (규칙 27 R4 · 규칙 35)**~~ → **✅ 2026-08-06 해소 — PO·트랜스퍼 공통** in-flight 잠금(`apply_lock_at/by`, 규칙 21). 🟡 **2026-08-10 부분 검증** — 같은 계정 반복 클릭에서 이중 이동 0회(진행 숫자가 단조 감소만 — 세션 문서 2장). **다른 계정 동시 클릭은 여전히 미검증.** 트랜스퍼는 duplicate 400 방어가 없어 동시 실행이면 진짜 이중 이동이었다 — 같은 날 확장의 이유. 📌 **검증 방법(2026-08-11 확정 — 세션 문서 9장)**: ⚠️ **트랜스퍼로 할 것**(역방향으로 되돌릴 수 있다 — PO stock received 는 되돌리기가 번거롭다) · A 가 OK 후 **20초 안에** B 가 OK(90초 후엔 만료 탈취라 판정 불가) · B 계정에 `perms.apply` 필요.
...
- **스캔 타임스탬프 기반 순수 작업시간 (규칙 37)** — 현재 소요시간은 근무시간 경과라 "열어두고 다른 일 한 시간"이 포함된다. 스캔 이벤트 타임스탬프 집계가 필요. ⚠️ 위 started_at 덮어쓰기 항목과 연결 — `picked_at` 이 픽 라인에 실제로 채워지는지부터 확인할 것(2026-08-11 실측: SO-14464-1 은 전 행 null 이었다 — 채워지지 않으면 이 항목의 전제가 무너진다).
...
| 2 | admin `wms_pick_tasks.select("status")` 배치 카운트 | ⬜ TODO — **이미 1000+ 로 과소 표시 중**. count 헤드 쿼리로 전환하면 캡 무관 |
| 3 | admin `wms_pack_tasks.select("status")` | ⬜ TODO — 997 임박. 동일 처방 |
...
| 11 | receiver 빈 지정 드롭다운 폴백 `wms_sku_bins.eq("warehouse")` | ⬜ TODO — sku_bins 는 창고당 수천 행(sku×bin) 가능. **1순위는 EF `action=bins` 라 실해 제한적**(폴백만 잘림) |
