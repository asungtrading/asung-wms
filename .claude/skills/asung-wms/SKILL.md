...
15. **Invoice First 게이트 fail-closed 전환 판단** (규칙 20 · 2026-08-05 사용자 결정으로 별건 분리). 지금은 **fail-open** 이다 — `InvoiceStatus` 를 못 찾으면 **통과**시킨다(400 이면 차단, 200 이지만 해석 불가면 묵과라는 비대칭 포함). 지금 fail-closed 로 바꾸면 해석 실패 하나로 **Apply 가 전부 막혀 창고가 멈출 수 있다** → **먼저 EF 로그로 실동작을 확인**할 것(해석 불가가 실제로 몇 건 나오나). 로그에서 0건이면 전환이 안전하다는 근거가 되고, 나오면 그 케이스를 먼저 고쳐야 한다.
