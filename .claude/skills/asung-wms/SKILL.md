...
비재고 캐시가 안 도는데 아무도 몰랐다. ✅ **2026-08-26 등록 완료**([실측] jobid 15 · `26 3 * * *`).
둘 다 4조건 충족: 에러 없음 · 정지 없음 · 신고 없음 · 데이터 틀림.

**08-25 「작업시간 자」 세션: 2건** (⚠️ 2026-08-26 정정 — 1 → 2. 세션 성격 = 깊이 판 구현+검증 —
같은 자를 전 워크플로에 완결하며 단계마다 조사 선행. 저녁에 그 조사가 결론을 두 번 뒤집었다.
경위 정본 `docs/sessions/2026-08-25-work-time-same-ruler.md`):
⚠️ **같은 날 세션이 하나 더 있다** — 원장 `docs/sessions/2026-08-25-transfer-line-deletion.md`
가 **별도 1건**(트랜스퍼 라인 소멸이 원장에 신호를 안 남김 · 토론토 재고 138 SKU 오차)을 자기
문서에 계상했다. 이 앵커는 **「세션당」**이라 두 수를 합치지 않고 세션별로 둔다 — 날짜로 묶어
"08-25 = 2" 로 읽지 말 것(**날짜 합계로 보려면 3**).
...
② **리시빙 라인 시각 붕괴**(`patchFor` 가 `putaway_auto` 외 모든 쓰기에 `last_received_at/by` 를
다시 찍어 **Place all 이 스캔 시각·사람을 덮는다** — [실측 receipt 67] 실제 **80분**이 라인
시각 **3.2초**(bin 별 버스트)로 남아 Throughput 이 **"< 1 min"** 을 표시. 4조건 충족 ·
2026-08-17 이후 전 리시빙 라인이 대상 · 소급 복구 불가).
⚠️ **②가 당일 계상에서 빠졌던 경위 (2026-08-26 정정)**: 발견 당시에는 「자(尺)의 문제 =
고칠 대상」으로 다뤄 미해결 목록에 넣었을 뿐 **「조용한 결함」으로 분류하지 않았다.**
저녁 재조사에서 성격이 드러났다 — ①과 **같은 형태**다: 둘 다 **나중 조작이 앞선 기록을 덮는다**
(재개가 vmethod 를, Place all 이 스캔 시각을). ⇒ 📌 **교훈: 「고칠 것」과 「이미 틀어져 있던 것」은
다른 축이다** — 수리 계획이 서면 분류가 끝난 것처럼 느껴져 계상을 건너뛰게 된다.
...
**09-01 세션: 0건** (세션 성격 = 잔여 항목 추적 → 장애 규명 → 원장 협의 — **깊이 팠으나 구현은 1커밋**.
경위 정본 `docs/sessions/2026-09-01-refresh-chain-dead.md`):
⚠️⚠️ **오늘 최대 발견(`⟳ Stock` 체인 12일 이상 반쪽 사망)을 「세지 않는다」고 판단했다 —
근거를 남긴다**(다음 세션이 같은 논의를 다시 하지 않게):
① **④ 데이터가 틀렸다 — 미충족(결정적)**: 06:30 정기 배치는 정상 작동해 값은 **설계된 만큼
(하루)만** 낡았다. **잘못된 값이 저장된 게 아니라 요청한 갱신이 안 온 것**이다
(08-12 PHASE 라벨을 「표시 오해지 데이터 오류 아님」으로 제외한 것과 같은 논리).
② **① 에러 없음 — 미충족**: `status='error'` + Exception 문자열이 **실제로 기록됐다**.
다만 아무도 안 읽는 자리에 났다(문자상 미충족, 취지상 애매).
③ ⭐ **선례가 결정적** — **08-14 CSV 7주 사고가 앵커에 계상되지 않았다.** 구조가 **동일**하다
(자동화가 조용히 죽음 → 데이터가 묵음 → Health 감시로 처방). 같은 모양을 이번에만 세면
지표가 일관성을 잃는다. (더 심각했던 08-12 팩 대기열 정지도 4조건 미충족으로 제외한 전례.)
④ **이 앵커가 재는 축이 아니다** — 정의상 「프론트엔드가 불변식을 소유하는 구조의 비용」인데
이번 건은 **GAS 트리거 한도**라는 다른 축이다. 섞으면 추세선이 흐려진다.
⚠️⚠️ **「안 센다」가 「덜 심각하다」는 뜻이 아니다** — 그 부류(**자동화가 조용히 죽는 것**)를
재는 앵커가 **아직 없기** 때문이다. 08-14 에 이어 **두 번째**다 — **세 번째가 나오면 지표를
만들 값어치가 있다.**
...
- ⚠️⚠️ **`⟳ Stock` 온디맨드 체인 — 반쪽만 죽을 수 있다 (2026-09-01 실사고 · 정본 `docs/sessions/2026-09-01-refresh-chain-dead.md`)**: admin 헤더의 `⟳ Stock` 버튼(`stock` perm·admin)은 **아무 일도 하지 않는다** — `wms_refresh_requests` 에 행 하나를 insert 하고(admin.html:343~364) 15초 폴링으로 그 행의 `status` 를 볼 뿐이다. 실작업은 GAS 1분 폴링 트리거가 집어가는 **2단 체인**(`gas-system-automation/Wmsstockrefresh.js`): ① `runBinStockSync()`(Cin7→BQ) → `sr_installStep2_()` 가 **원샷 트리거를 새로 만들어** → ② `sr_step2_()` → `runWmsMasterSync()`(BQ→Supabase).
  - ⚠️⚠️ **②의 이음새가 「트리거 생성」이라는 유한 자원이다.** GAS 트리거 **20/20 한도**에 걸리면 `create()` 가 `too many triggers` 로 throw → catch 가 `status='error'` 를 찍는데, **①은 이미 성공한 뒤**다. ⇒ **BQ 는 최신, Supabase 는 그날 06:30 값** 이라는 어중간한 상태가 남고 **어느 화면에도 표시되지 않는다.** [실측] 최근 5건 전부 이 에러 — **8/20~9/1, 12일 이상**(그 이전은 미확인).
  - ⚠️ **매일 06:30 정기 배치(`setupWmsSyncTrigger`)는 살아 있었다** — 이미 설치된 트리거라 한도와 무관. 죽은 것은 **「지금 갱신해줘」 경로 하나**이고, 그것이 **당일 입고를 반영할 유일한 수단**이다.
  - 📌 **실패는 「그 순간 admin 탭을 보고 있던 사람」에게만 보인다** — 탭을 닫으면 15초 폴링이 멈춰 토스트도 못 본다. **12일간 아무도 신고하지 않았다** ([실측] 창고 담당자가 8/20·8/21·8/24 세 번 누르고 8일간 아무도 안 눌렀다 — **추론**: 세 번 실패하면 사람은 신고하는 대신 **도구를 그만 쓴다**. 「신고 없음」을 「문제 없음」으로 읽지 말 것).
  - **수동 우회**: BQ 가 이미 최신이면 GAS 편집기에서 **`runWmsMasterSync` 만** 실행(1~2분). 버튼 재클릭은 ①부터 다시 돌아(3~7분) 같은 지점에서 또 끊긴다.
  - ⚠️ 2026-09-01 에 트리거를 18개로 줄여 임시 해소했으나 **여유가 2개뿐이라 재발한다** — 감시 검사는 백로그(「신규 기능」).
...
## 규칙 9 — 프론트엔드 8화면 (순수 HTML/JS + Supabase CDN)
...
- **bin = GUID.** `From`/`To`/`LocationID` 에 bin 이름("창고: bin")을 넣으면 400 "not found in Locations reference book". `/ref/location` 에서 GUID 조회 (EF `binGuid()`/`binMap()`).
  - ⚠️ **`/ref/location` 은 Total 2678 인데 Limit 500 로 잘린다. bin GUID 는 최상위 창고 행(`ParentID` 없음)의 `Bins[]` 에서 뽑아라**(에드먼튼 628·토론토 2047 전부 포함 — 페이지네이션 불필요). ~~**child-location 의 `Name` 은 bin 이름이 아니다**(예 "071164313169" 바코드류 — 매칭 불가한 죽은 경로였고 제거했다).~~ → ⚠️⚠️ **[정정 2026-09-11] child 행의 `Name` 은 bin 이름이 맞다**(`ref/location` 전량 2,678행 실측 · 하위 2,675행 `Name` 이 `Bins[]` 와 2,675/2,675 일치 · 바코드 0). ⭐ **폴백이 죽은 이유는 이름이 아니라 `Limit 500` 잘림**(에드먼튼 child 0행)이었다 — 근거가 틀렸던 것이고 **처방(`Bins[]` 만 쓴다)은 그대로 유효**하다. `binGuid` 코드는 건드리지 않는다. 상세 `cin7-api/references/stock-write.md` 5절. 실측 2026-07-28 **TR-02935**: 잘린 500행에 에드먼튼 child 가 0행이어서 bin GUID 조회가 첫 호출부터 throw → Apply 의 bin 이동이 **한 건도 실행되지 않고** 전 품목이 집결 bin EZ010101 에 남았다. 토론토도 우연히 앞 페이지였을 뿐 안전하지 않았다. 이름 비교는 `trim().toUpperCase()`, `IsDeprecated` 제외. `?action=bins` 도 같은 소스.
...
- **픽리스트 배송 주소·Terms (2026-08-30)**: sale 상세 최상위 `ShippingAddress`(객체)·`Terms`(문자열, 실측 SO-15505) → `wms_orders.ship_address`(jsonb 원문)·`terms`(text) → batchPage 좌열에 Terms + Ship To(**`DisplayAddressLine1/2` 그대로** — 조립 금지). 목적 = 인보이스 처리 전 눈 확인. ⚠️ **유입 시점 값**(A안) — 인쇄 시점 최신값 아님, 최신화(hold 재조회 patch 에 얹기)는 다음 단계. wave **요약 테이블에는 안 넣는다**(멤버 페이지에 각자 인쇄). 상세는 `references/schema.md` 2026-08-30 절.
...
- ⚠️ **reaper(`wms_reap_stale_claims`)는 리시빙을 못 다룬다 — 구조적 이유**: `wms_receipts` 에 assigned_to·work_started·heartbeat_at 이 아예 없고 되돌릴 pending 풀도 없다. 리시빙은 소유권 없는 공유 문서 설계(위 "나눠 받기"가 기능)라 **reaper 가 풀어줄 "점유"가 데이터 모델에 존재하지 않는다.** (2026-08-24 reaper 를 흡수한 **자동 Hold(`wms_auto_hold`)도 같은 이유로 리시빙 무관** — 점유가 없으니 풀 것도 없다.)
...
**✅ 2026-09-10 — 같은 사람 · 다른 기기(탭) 차단 (① 픽·팩 · ✅ 같은 날 밤 배포(`23df53a`) + 현장 검증 3종 통과: 다른 탭 → 프리즈 모달 실물 · Reload 되찾기 대칭 · 단일 탭 Ctrl+Shift+R 오탐 없음 · 리시빙은 하루 관찰 후 별건)**
- **구멍**: 위 가드는 전부 `assigned_to` **이름 비교**라 **같은 사람이 같은 배치를 두 기기에서 열면 통과**했고, 완료·Hold RPC 가 그 화면의 로컬 스냅샷으로 전 라인을 덮어써 다른 기기의 픽이 0 으로 돌아간다(두 사람이면 CAS 가 막는데 같은 사람만 뚫렸다 · 실해 0건 — 지문 SQL 은 세션 문서 §2). [Caleb 확정 — 다시 논의하지 말 것] ① 막는다 ② **새 기기가 이기고 옛 기기는 얼어붙는다**(로그인은 안 막는다 — 배터리 꺼진 태블릿은 로그아웃을 못 한다) ③ **작업 단위 잠금, 사람 단위 아님** — 같은 사람·다른 배치 두 기기 · admin+picker · picker+receiver 병행은 허용(유실은 같은 작업 행에서만 난다). 규칙 문장(세 화면 공통): **「한 작업(배치·wave·팩)은 한 화면에서만 열린다.」**
- **정체 = `wmsAuth.sessionId()`** (wms-auth.js) — `sessionStorage["wms_session_id"]` UUID · 하드 리로드에 유지 · 탭·기기마다 다름 · 탭 닫으면 소멸 · 저장 불가면 메모리 폴백. ⚠️⚠️ **CLAUDE.md 「상태를 localStorage 에 두지 말 것」과 충돌하지 않는다 — 「상태 vs 정체」**: 그 규칙은 *작업 상태*(held_by 등)를 기기에 두면 다른 태블릿에서 깨진다는 것이고, 이 값은 *이 화면(탭)의 정체* 라 정의상 그 탭에만 있어야 한다. **상태(어느 화면이 잡고 있나)는 서버 컬럼** `session_id`(`wms_pick_tasks`·`wms_waves`·`wms_pack_tasks` · nullable · `20260910230704_session_guard_pick_pack.sql`). 릴리스(Hold·자동 Hold·admin 롤백)에서 지우지 않는다 — 모든 클레임이 덮어쓴다. **규칙 위반으로 보고 되돌리지 말 것.**
- **클레임 = 쓰는 순간**: `assigned_to:me.name` 을 싣는 UPDATE/INSERT **11곳 전부**(picker 7 · packer 4)에 `session_id:SID` 동봉 + **종전 읽기만 하던 재개 경로**에 `claimSession(kind,id)`(picker: resumeBatch · restoreFromUrl task · resumeWave(restoreFromUrl wave 도 수렴) · scanTakeover 「내 것」) / `claimSessionPack(id)`(packer: resumePack in_progress 분기 — scanTakeoverPack 내 것·restoreFromUrl·takeoverPack 수렴점) 신설 — 조건부 UPDATE(`assigned_to=me.name and status='in_progress'`) + `.select()` 1행 판정(규칙 20/24), 0행이면 목록으로 · **네트워크 실패는 통과**(best-effort — 서버 session_id 가 옛 화면 것이라 완료가 other_device 로 얼어붙고 Reload 가 되찾는다 = 유실 없는 쪽으로 실패). ⚠️ 한 곳이라도 빠지면 그 경로만 조용히 뚫린다(startWave 13일 전례). 근거 grep: `grep -n "assigned_to:me.name" picker.html packer.html | grep -v session_id` → **0줄**이어야 한다.
- **검사**: `checkOwner` 가 `session_id` 도 읽어 `who===me.name && row.session_id && row.session_id!==SID → {s:"lost",who,otherDevice:true}` → `lostMsg(r)` 가 전용 문구 `otherDeviceMsg()`("This {batch|wave} is now open on another device that is signed in as you. This screen is out of date and won't save. Continue on the other device — or tap Reload here to take it back to this one.")를 고른다. Reload → restoreFromUrl → claimSession 이 이 탭으로 되찾고 **저쪽이 얼어붙는다**(「새 것이 이긴다」 양쪽 대칭). `row.session_id null` = 배포 전 클레임 → 통과.
- **최종 관문 = RPC 4개 CAS**(`wms_complete_pick`·`wms_hold_pick`·`wms_complete_pack`·`wms_hold_pack`): 바꾼 것 셋만 — (a) 마지막 파라미터 `p_session_id text default null` (b) CAS where 에 `and (p_session_id is null or 행.session_id is null or 행.session_id = p_session_id)` (c) 0행 반환에 `reason`(같은 사람·in_progress·세션만 다르면 `'other_device'`, 그 외 null). 프론트는 payload `p_session_id: SID` · `res.reason==="other_device"` 면 **재조회 없이** 전용 프리즈(`completeCasFailed`/`holdCasFailed` 앞 분기). ⚠️ wave 멤버 UPDATE 에는 세션 조건 없음(레거시 wave 「n of m matched」 예외 방지 — wave 행이 소유권 단위). 함수 원문은 통째 복사 → diff `+14/−3 · +14/−3 · +8/−2 · +8/−2`(교체 원줄 10 = 파라미터 4 + 0행 return 6 · v5_4 §2).
- ⚠️⚠️ **시그니처 교체 = 옛 시그니처 `drop function` 필수** — 안 하면 오버로드 둘이 남아 PostgREST 가 옛 호출(p_session_id 생략)을 모호하다고 거부 ⇒ **완료·Hold 전면 실패**. drop → 재생성 → revoke/grant 를 **한 마이그레이션 안에서**(drop 뒤 ACL 미보존). **배포 순서 DB → HTML 절대**(옛 HTML 은 `default null` 로 종전 동작 · 새 HTML 이 옛 RPC 를 만나면 400). 📌 2026-09-10 첫 구현은 원장 마이그레이션 13건이 큐에 있어 DB 를 못 올려 **파일 단위로 되돌렸다** — 남의 모듈과 마이그레이션 큐를 공유하면 내 배포 시점이 남의 준비 상태에 묶인다. 시그니처가 바뀌는 RPC 는 큐가 비어 있을 때 넣는다(경위 `docs/sessions/2026-09-10-same-user-multi-device-guard-reverted.md`).
- 검증 격자 `scripts/testdb/session_guard_check.sql`(로컬 · 트랜잭션 안 적용 → 격자 → ROLLBACK · `-v mig=<마이그레이션|/dev/null>`): 오버로드 1개 · ACL · `p_session_id` 생략 회귀 · 일치/불일치(0행+reason · 라인·discrepancy·status 무변) · 레거시 null 통과 · 남의 배치 reason null · wave · 팩 완료/Hold · 새 화면 클레임 뒤 완료. 설계 정본 `docs/sessions/2026-09-10-multi-device-guard/v5_3-design.md`(판정 6개 · 대안 기각). ⬜ 2단계 리시빙(`wms_receipt_sessions (receipt_id, worker)` — 여러 사람 허용·같은 사람은 하나) · 별건: LIVE NOW ×2 표시 · 리시빙 같은 라인 CAS.
...
⚠️⚠️ **avg time·min/line 의 신뢰 한계 (2026-08-11 조사 — 코드로 확정된 사실)**: `started_at` 이 실제 작업 시작이 아닐 수 있다. **[확정 — 코드]** ① **스캔 이어받기**(picker `openByScan` 이어받기 분기 · packer 스캔 이어받기)가 `started_at` 을 **지금으로 덮어쓴다** — 인계자의 작업 시간이 소멸하고 완료자의 잔여 시간만 남는다(과소). confirm 문구 "quantities are kept" 는 수량만 참이다. ② **Hold→재개**: Hold RPC 는 started_at 을 의도적으로 보존하는데(마이그레이션 주석 명시) **재개가 `startBatch`/packer 큐 클레임을 재사용해 덮어쓴다**(held 카드 디스패치의 else 분기 — RPC 가 지킨 것을 재개가 무효화). ③ **wave 멤버**는 반대 방향 — `startWave` 가 전 멤버에 wave 시작 시각을 일괄 기록 + 완료도 일괄이라 **멤버 전원의 dur ≈ wave 전체 소요**(과대). **대조군(보존이 옳게 된 곳)**: `takeoverBatch`(방치 클레임)는 assigned_to·heartbeat 만 갱신 — 같은 "이어받기"인데 스캔 쪽만 리셋(설계 갈라짐). **[확정 — 증언] (2026-08-11 후반)**: SO-14464-1(68라인/218유닛을 2분 10초 기록)의 원인은 **② Hold→재개**로 확정 — ⚠️ **근거는 코드·행 데이터가 아니라 담당자 증언이다**("라인 수가 많아 중간중간 Hold/Resume 을 여러 번 했다. 2분에 끝날 특별한 상황은 없었다"). 행에는 판별 흔적이 없었다(`picked_at` 전 행 null · 롤백 로그 없음) — 증언이 유일한 판별 근거. ✅ **①②는 2026-08-11 수정 완료**: **"started_at 은 최초 시작이다 — 이어받기·재개가 덮어쓰지 않는다"** 가 명문 원칙이다. 4곳(picker 스캔 이어받기·startBatch / packer 스캔 이어받기·resumePack 클레임) 전부 SET 에서 started_at 제거 + `ensureStartedAt`/`ensureStartedAtPack` 헬퍼의 서버측 only-if-null backfill(`.is("started_at",null)` — 경합에도 최초 1회만). 최초 시작/재개의 구분자는 행의 null 여부(롤백은 null 리셋·Hold 는 보존 — 모드 분기 불필요). packer `startPack` 의 insert(진짜 최초)는 무접촉. ⚠️ **[정정 2026-08-24] 그 4곳에 wave 경로가 빠져 있었다** — `startWave` 는 wave 행·멤버 전원의 started_at 을 계속 덮어쓰고 있었고(②의 wave 판 — Hold→재개 리셋 = 과소), 작업시간 측정 개편 조사에서 발견해 같은 날 수정(멤버 SET 에서 started_at 제거 + `.eq("wave_id").eq("assigned_to",me.name).is("started_at",null)` 일괄 backfill — ensureStartedAt 와 같은 서버측 only-if-null 보장 + wave CAS 가 승자 1명만 도달시키는 이중. wave 행의 started_at:now 는 소비처 0 실측이라 무접촉, held_by:null 만 추가 — holdCasFailed 판정 전제와 실물 일치). ⚠️ ③ wave 멤버 과대(일괄 기록 → 멤버 전원 dur ≈ wave 전체)는 **이번 수정과 별개고 여전히 잔여**다(배분 설계 필요 — 백로그): 이번에 고친 것은 ②의 wave 판(과소) 하나뿐이고, 신규 wave 의 전원 동일 시각 기록(③의 원인)은 backfill 이 그대로 유지한다. ⚠️⚠️ **수정의 부작용 — 방향 전환(과소→과대)**: 이제 avg 는 **Hold 대기 시간을 포함**한다(3시간 방치 = 3시간 기록). 매니저가 "갑자기 느려졌다"고 읽으면 오해다 — **기록이 정확해진 것**(2026-08-11 이후 데이터부터). 화면 문구는 **넣지 않기로 판단**(부제가 이미 길고, 일회성 전환이라 영구 문구는 과함 — 물으면 이 문단이 답). ⚠️⚠️ **24시간 캡 의존성(건드리기 전에 읽을 것)**: `dur()` 는 24h 초과를 이상치로 버리는데, 이 캡이 **밤샘 Hold 배치를 avg 에서 자동 제외하는 이번 수정의 안전장치**다 — 동시에 **통계 공백**이기도 하다(오래 Hold 된 배치는 avg 에 아예 안 잡힌다: "왜 이 배치가 avg 에 없지"의 답). 캡을 올리면 밤샘 Hold 가 avg 를 오염시키고, 없애면 며칠짜리 dur 이 들어온다 — ~~순수 작업시간이 생기기 전엔 캡 유지~~ → ✅ **[2026-08-25 C 병기] 캡을 「자에 맞게」 옮겼다**: 옛 자(벽시계)는 기존 캡(`raw<24h`) 그대로 · **새 자(minus holds)는 「공제 후 값」에 캡(`net<24h`)** — 캡의 존재 이유(밤샘 Hold 오염 방어)가 새 자에선 공제 자체로 해소되므로, 밤샘 Hold 배치는 옛 자에서 빠지고 새 자에는 정확한 값으로 남는다([실측] pick 1767: 989.7분 → 30.8분). 분모가 달라지는 경우(옛 캡 초과 배치)만 `(n=N)` 병기 — 조용히 다른 분모 금지. → 매니저 안내: ③이 섞인 기간의 avg·min/line 은 여전히 개인 비교 근거로 쓰지 말 것.
⚠️⚠️ **pack `verification_method` 는 `2026-08-26 00:00:00+00` 이후만 신뢰** ([실측 2026-08-25] 원격 반영 확인 — 두 RPC prosrc 에 coalesce 확인 · 배포 19:51:36+00 · **마지막 null 발생 19:45:34+00**. ⚠️ 신뢰 시점을 배포 시각이 아니라 하루 여유로 둔 이유: GitHub Pages 배포와 태블릿 새로고침 사이에 창이 있어 배포 직후 완료된 팩은 **옛 프론트가 보낸 값**일 수 있다 — RPC 심층 방어가 null 은 막지만, 그 라인의 **기존값 자체가 이미 오염**돼 있었을 수 있으므로 여유가 안전하다): [실사고 2026-08-25] 완료·Hold RPC 가 전 라인의 방법을 `r.vm` 그대로 다시 쓰는데 packer 재개(resumePack)가 verified_base 는 복원하면서 **vmethod 만 null 로 버려** — 스캔으로 저장된 라인이 Hold·재개·새로고침 뒤 완료되면 null 로 덮였다(분포: scanned_base 18,364 · **null 3,702** · manual 3,449 · scanned_variant 2,687 — null 이 manual 보다 많고 당일까지 생성). **Hold 를 성실히 쓴 사람일수록 기록이 더 지워졌다.** 이 컬럼은 「실물 스캔 없이 눌러 넘긴 작업」을 찾는 직접 증거 — 과거 분포로 그 판정을 하지 말 것(덮인 스캔과 진짜 비스캔이 구분 불가 · 백필 금지). 수정 = packer 복원 2줄(픽과의 비대칭 해소 — 픽은 두 경로 모두 원래 복원) + RPC 심층 방어 `coalesce(r.vm, 기존)`(20260825193231 — verified_at 보존과 같은 수법 · 새 값이 항상 이겨 정당한 변경은 통과 · 미래의 「상태를 잃는 경로」 대비 마지막 벽). picker 는 이 구조 결함 없음(코드 확인).
🆕 **리시빙·트랜스퍼도 같은 자 (2026-08-25 · 마이그레이션 `20260825203457` — Receiving 표 "Avg receive − holds" 병기)**: `(completed_at − created_at) 순수 벽시계 − Hold·선적대기(partial) 공제` — ⚠️ workMinutes 와 섞지 않는다(주말·밤 이중 공제). 기록 = `wms_task_holds` 재사용(`task_kind`+'receipt' · `source`+'partial' — partial 은 「이 선적분 완료·나머지 대기」: Apply 게이트가 completed 만 허용 + `cin7_purchase_id` 유니크라 PO 당 receipt 1건 = 다회 선적을 한 receipt 에 누적 후 Apply 1회 · I&R 하나 설계의 양면). 기록 RPC = `wms_pause_receipt`(플립+이력 원자 · CAS in_progress 만). **닫기 3곳**: receiver 재개(openReceipt) · **admin Reopen**(DB 직접 플립이라 receiver 분기 미발동 — 별도 호출) · **완료 시**(⚠️ 리시빙 고유: A 가 Hold 한 문서를 동시 작업자 B 가 그대로 완료 가능 — hold 종점 = 완료 시각). ⚠️ **문서 단위 지표** — 여러 명이 같은 receipt 를 공유(assigned_to 없음)하므로 「사람의 시간」이 아니다(화면 각주 "per receipt, not per worker" · **사람별 시간은 못 낸다 — 2026-08-25 재조사 확정**: 주범은 ~~저장 큐 flush~~ 가 아니라 **Place all 재스탬프**다[실측 receipt 67 = PO-01151: 20줄 시각이 bin 별 버스트 3.2초 — patchFor 가 putaway_auto 외 모든 쓰기에 last_received_at 을 덮는다]. 라인 시각 한 칸(마지막 터치)으로는 스캔 리듬이 안 남아 Throughput 사람별 work time 표기를 같은 날 제거했다 — 아래 「작업자 통계는 …」 항목 2026-08-25 개정 참조. 되살리기 선행 = kind 별 축 분리 — **✅ 2026-08-26 완료**(`20260826014848` · 아래 「리시빙 축 분리」 절), 화면은 데이터 축적 후 별건). ⚠️ **자동 Hold 미적용** — 점유 모델 부재(reaper 가 리시빙을 못 다룬 것과 같은 뿌리). **잔여 위험: Hold 를 안 누르고 열어둔 채 다음날 완료하면 그 receipt 의 새 자는 과대**(수동 Hold·partial 에 의존). 신뢰 시점 = **2026-08-26 00:00 UTC**(배포 다음날 0시 여유 상계 — verification_method 전례와 같은 원칙) · hold_leak 이 receipt 판으로 확장(정상 열림 = held/partial · 삭제 receipt 는 안 잡음 · 이중 open 은 잡음 — 테스트 `wms_pause_receipt_test` 7케이스). ✅ **검증 닫힘 (2026-08-26 · 「검증 대기」에서 내림)**: [실측 토론토 12:29 · `verified_at >= '2026-08-26'`] **scanned_base 587 · scanned_variant 63 · manual 43 · `null` 0건**(표본 693) — 어제까지 3,702행씩 쌓이던 null 이 **0**이다. **못 본 경로는 없었다**(복원 2줄 + RPC coalesce 가 둘 다 작동). 📌 **부수 수확 — 처음으로 믿을 수 있는 비스캔 비율 6.2%(43/693)**: 오염 상태에서 계산되던 `(3,702+3,449)/28,202 = 25.4%` 는 **덮인 스캔을 비스캔으로 세고 있었다**. 실제 값은 **픽의 11.5% 보다도 낮다** — 「팩이 스캔을 덜 한다」는 그림이 반대로 뒤집혔다. ⚠️ **이 컬럼으로 사람·관행을 판단하기 전에 신뢰 시점을 먼저 볼 것**(위 「과거 분포로 판정 금지」의 실물 사례 — 4배 부풀려진 값으로 현장을 의심하던 중이었다). ⬜ **남은 것: manual 43건의 성격**(「스캔 후 수량만 탭」 vs 「안 보고 넘김」)은 여전히 구분 불가 — **스캔 횟수 기록**(별건 보류)이 있어야 갈린다. 비율이 낮은 것이 이 항목을 닫아주지 않는다. 경위 정본 = `docs/sessions/2026-08-25-work-time-same-ruler.md` 「2026-08-26 검증」 절. 재확인 쿼리:
```sql
-- verified_at 필터가 안전한 근거: 완료 RPC 가 coalesce(l.verified_at, v_now) 로 항상 채우므로
-- 완료된 팩 라인엔 verified_at 이 반드시 있다(20260821201104:95) — 미완료 라인은 아직 판정 대상 아님
select id, pack_task_id, verified_base, verified_at from wms_pack_task_lines
 where verified_base > 0 and verification_method is null
   and verified_at >= '2026-08-26' order by verified_at desc;
```
🆕 **Hold 구간 추적 — A 기록 / B 자동 Hold (2026-08-24 · 배포 완료) / C Stats 병기 (2026-08-25 — 아래 갱신)**
— ⚠️ **경위·설계 판단의 정본은 `docs/sessions/2026-08-24-hold-tracking.md`**(방향 전환 2건: 「스캔 갭 컷」→「elapsed − Hold 구간」 · 「reaper 수정」→「흡수 폐지」 · 내가 틀렸던 것 2건). 여기엔 **모르면 사고가 나는 것만** 남긴다:
  ...
  · ⚠️⚠️ **`hold_leak` 첫 실물 + 구조적 함정 — ⬜ 판단 대기 (2026-08-26 · 다음 세션이 반드시 볼 것)**: [실측] `id 45` · receipt 70(TR-03975) · `held_at 08-25 21:42:55` · `resumed_at null` · `task_status = in_progress`. 경위 = 08-25 21:42 Hold(새 코드 · 이력 정상) → 21:59 **옛 화면으로 재개**(닫기 호출 없음) → 08-26 14:09 다시 열어 스캔(축 기록 정상). ⚠️ **그런데 다시 열었을 때도 안 닫혔다** — 닫기는 `openReceipt` 의 **「상태가 `in_progress` 가 아닐 때」** 분기에서만 도는데 이미 `in_progress` 라 그 분기를 지나지 않는다. ⇒ **한 번 놓치면 그 문서를 완료할 때까지 영원히 안 닫힌다**(닫기 3곳 중 receiver 재개 경로의 사각지대 — admin Reopen·완료 시는 별 경로). ⬜ **판단**: 고칠 구조인가, 아니면 「완료 시에도 닫기」가 마지막 안전망이라 수용인가? **판단 재료 = TR-03975 완료 시 실제로 닫히는지**(그 경로의 첫 실물 검증) — `select id, receipt_id, held_at, resumed_at from wms_task_holds where id = 45;`. 경위 정본 = `docs/sessions/2026-08-25-work-time-same-ruler.md` 「2026-08-26 검증」 절.
...
- → ✅ **[2026-08-24 종결] 그 판단이 실행됐다** — 라인 시각(A 단계 이력) 위에서 **자동 Hold(B)가
  reaper 를 흡수·폐지**해 이 조합(reaper × backfill) 자체가 소멸했다. started_at 은 이제 어떤
  서버 경로도 지우지 않는다(무작업 백아웃·rollback 리셋만 null — 정당). 정본 = 규칙 37 계열
  「자동 Hold 가동」 항목 + 마이그레이션 `20260824202539` 헤더.
...
- **작업자 통계는 별도 표가 아니라 「Throughput by worker」에 통합**한다(같은 사람이 픽·팩·리시빙을 다 하므로 표가 갈라지면 비교가 안 된다). 픽/팩 집계와 리시빙 집계를 **도착 순서 무관하게 이름 합집합으로** 합쳐 그린다. ⚠️ **2026-08-17 부터 리시빙 사람 귀속은 라인 단위**(`last_received_by` — 규칙 24 라인별 작업자 항목): 폴백 그룹(도입 전 라인) 키는 **`~이름`** 으로 분리 집계(의도 — 그 사람 실적이라 단정할 수 없다). ~~렌더에서도 별도 행~~ → ⚠️ **2026-08-20 개정: renderProd 가 실명 행에 접어 넣되 수치는 `(+N attributed)` 로 분리 표기**(같은 사람이 `Ian Lee`/`~Ian Lee` 두 줄로 보여 혼란 — Caleb 결정. 접두는 `RECV_FB_PREFIX` 상수를 생성·파싱이 공유). **한 숫자 합산은 여전히 금지** — PO-01131 형 거짓의 부활. 본문 수치(units·receipts·putaway%)는 정확분만, attributed 는 괄호·연한 막대·펼침 라벨 `(attributed)` 로 · ~~work time = 사람별 min~max `last_received_at`~~ → ⚠️⚠️ **2026-08-25 제거(결론 ⑤ — "이 지표는 사람별로 낼 수 없다")**: Place all 재스탬프가 스캔 시각을 덮어([실측] receipt 67 — 실제 80분이 3.2초 bin 버스트로) 사람별 시간이 구조적으로 붕괴한다. 리시빙 세그먼트는 **작업량만**(라인·유닛·receipts·putaway%) 표기, 펼침 목록도 시간 열 제거(문서 시간 대체는 기각 — 사람 행 아래 펼침에 문서 시간을 넣으면 표기를 붙여도 자리가 거짓말을 한다 · At 열 = 그 사람의 마지막 터치 "Last touch"). 시간의 정본 = Receiving 표 "Avg receive − holds"(문서 단위). 대신 **Receiving 표에 `Workers (last touch)` 열 신설**(이름+담당 라인 수 내림차순 · 7명 이상 = 상위 5 + "+M more" + 셀 title 전체 명단 · NULL 라인은 이름 없이 "N unattributed" — received_by 귀속 금지). ⚠️⚠️ **표 아래 Workers 각주가 이 열의 핵심 안전장치다**(마지막 터치 기준이라 Place all 로 **사람·날짜가 둘 다 이동**할 수 있다 — 기간 귀속도 last_received_at 이므로 금요일 스캔분이 월요일 실적이 된다). **각주를 지우면 「참여자 목록」이라는 새 거짓이 생긴다** — 사람별 시간을 없앤 것과 같은 종류의 거짓이다. 받은 라인 0 인 receipt 는 byWorker 에 안 잡힌다.
...
- **hold 카드도 종착(`voided`·`closed`) 제외** (2026-09-01 — 실사고 SO-15688: Cin7 `Cancelled` → `unexpected` → ⊘ Void 후 카드 영구 잔존. 감지가 종착을 무시하므로 자동 해소가 영영 안 온다) — void 카드와 동형의 **렌더 필터**로 해소. ⚠️ hold_* 컬럼은 지우지 않는다(⊘ Void 도 무접촉 — "Cin7 이 언제부터 이상했나"의 기록, `cin7_void_state` 보존과 같은 이유). **왜 제외해도 안전한지 포함 정본은 `admin.html` `renderHolds` 도입부 주석.**
...
② 캐럿 펼침에 p_warehouse 를 넘기지 않는다 (null 명시)
   창고 필터가 걸려 있어도 그 SKU 의 전 창고 칸을 보여주는 것이 WMS 가 원장에 한 약속이다.
   ⚠️ 함수는 이것을 막지 않는다 — 화면에만 있는 규칙이다. 넘기면 그 창고 칸만 나오고 에러가 안 난다

③ diff 합만 보면 거울 칸이 사라진다
   +3/−3 이 SKU 층에서 0 이 된다(실물 PRO00124 · EB010302/EB010304 · diff 0 · diff_bins 2 · abs_diff 6).
   diff_bins · abs_diff 를 반드시 함께 그린다 — diff 가 0 이어도 diff_bins > 0 이면 경고(빨강 + 배지)
   ⚠️ 이게 없으면 SKU 층은 어긋남을 감추는 화면이 되고 칸 층보다 나빠진다

④ new_since_snapshot_bins 를 경고로 띄우지 않는다
   낮에 자리 이동이 있으면 늘 생기고 다음 스냅샷에 사라지는 정상값이다
...
⑤ 카드와 목록은 다른 시점을 센다 — 값이 달라도 정상이다
   ...
   목록 = inv_stock_master → inv_balance_vs_cin7(현재 뷰)
   [실측 09-02] 카드 21 · 목록 9 — 12칸은 낮에 해소된 것. 둘 다 맞다
   ⚠️ 이것을 결함으로 오인해 파고든 일이 실제로 두 번 있었다(WMS·원장 각 1회)
   ⇒ 카드 라벨이 「AS OF <스냅샷 시각>」 이고 부제가 「now N」 인 이유(51279b6). 「resolved」류 해석은 붙이지 않는다

⑥ 창고 값은 Cin7 원문 정확 일치 — 한 글자 틀리면 조용히 빈 결과
   Asung Trading Inc. / Asung - Edmonton / IN_TRANSIT   (2026-09-04 실측 확정 · <option value> 가 그 원문)
...
- ✅ picker/packer **heartbeat 전면 제거** → **스캔 이어받기**로 대체. admin BATCH ACTIVITY presence 기반(active/away). realtime presence·LIVE NOW 유지. ~~reaper 는 느슨한 백업~~ → 2026-08-24 자동 Hold 로 흡수·폐지(규칙 37 계열).
...
### ⚠️ 리시빙 축 분리 — ✅ 완료 (2026-08-25 확정 → 2026-08-26 구현)

⚠️ **경위·설계 판단의 정본 = `docs/sessions/2026-08-25-work-time-same-ruler.md` 「저녁 이어짐」**
(결론이 두 번 뒤집힌 기록 — ①「스캔 캡처가 선행」이 틀린 진단이었다 ②「사람 수를 못 세니 접자」가
성급했다: 축을 나누면 Place all 은 풋어웨이 축만 덮는다. 두 번 다 **「목적에 닿는가」**를 물어
방향이 정해졌다 — 목적 = **인력 배치**「몇 명이 몇 분」). 여기엔 모르면 사고가 나는 것만 남긴다.
...
✅ **신뢰 시점 = 실측 시각 그대로 (여유 상계 없음 — 2026-08-26 판단)**:
`last_qty_at` 첫 행 **2026-08-26 02:20:30.966+00**(id 5547 · Caleb Chang · + 버튼 · 같은 행의
`last_received_at` 이 동일 값 = **진실화 실증**) · `wms_receipt_stage_events` 첫 행
**2026-08-26 02:20:33.365781+00**(id 1 · receipt 76 · putaway → id 2 receiving 02:20:41 =
오간 순서 정상) · ⚠️ 그 행의 **풋어웨이 축은 null** — **수량 축이 안 덮인다는 것이 실물로 확인**됐다
(축 분리 설계의 핵심 전제 · receipt 67 붕괴가 재현되지 않음).
⚠️ **앞선 두 건(verification_method·RECV_PAUSE_SINCE)과 성격이 다르다 — 그래서 상계를 안 한다**:
저 둘은 옛 화면이 **틀린 값을 써 넣는** 창이라 하루 여유가 필요했지만, 축 컬럼·전환 이력은
옛 화면이 키를 **아예 안 보내 null 로 남을 뿐** 오염이 구조적으로 불가능하다. 여유를 두면
그만큼의 **진짜 기록을 버리는 손해**만 생긴다(안전 방향이 반대다). ⚠️ 단 `last_received_at`
**진실화**는 옛 화면이 종전대로 전송 시각을 보내는 창이 있다 — 그러나 그것은 **종전 동작 그대로**라
새 거짓이 아니고, 소비처가 날짜 단위(기간 귀속)라 실질 차이도 없다(온라인 쓰기는 초 단위 차).
⬜ **화면·Stats 소비는 별건**(데이터 며칠 축적 후): 계산 정의(리시빙 구간 = created_at ~
max(last_qty_at) · 풋어웨이 구간 = min(stage='putaway' at) ~ completed_at · 겹침 병기·합산 금지 ·
사람 수 = 축별 distinct)의 정본 = 마이그레이션 `20260826014848` 헤더. **판단 항목 4개와 확인
SELECT 3종은 세션 문서 「⬜ 다음」 절**(Hold·partial 공제를 어느 단계에서 뺄지 · Workers 열을
축 기반으로 갈아탈지+각주 재작성 · 복귀 빈도 실측 후 차단 판단).
⚠️ **개인 평가 목적은 보류 (Caleb 확정 — 다시 논의하지 말 것)**: ① 여러 명이 나눠 하고 Place all 이
귀속을 덮어 개인 몫을 가를 수 없다 ② **빠른 것이 좋게 표시되면 리시빙에서 빨라지는 가장 쉬운
방법이 「스캔 건너뛰기」** — 지표가 측정하려던 품질을 스스로 망가뜨린다(순위 미표시 결정과 같은 계열).
...
- **리시빙 「받기 끝」 기록(Done receiving) + kind 별 축 분리 + last_received_at 진실화.**
  ...
  설계 확정분:
  1. **`Done receiving` 버튼** — 「받기 끝」을 선언·기록. 미달 라인이 있으면 확인 창(실수 방지). ⚠️ **되돌아가기는 막지 않는다** — 리시빙 롤백이 존재하지 않아(Caleb 확정) 막으면 실수 한 번에 전 라인 재스캔이고, 그런 벌칙은 작업자가 버튼 누르기를 주저하게 만들어 결국 「받기 끝」 시각이 부정확해진다(**정직한 기록을 벌주면 기록이 사라진다** — 「풋어웨이 하드 게이트 재검토」와 같은 근거). **복귀도 기록**해 빈도를 실측한 뒤 판단. 분할 선적 재개는 실수가 아니라 정상 흐름 — 이 설계면 롤백 없이 처리된다.
  2. **kind 별 축 분리** — receiver `patchFor` 가 이미 qty/putaway 쓰기를 구분하므로(:1376-1383), 새 컬럼(예: `qty_touched_at`/`putaway_touched_at`)을 kind 별로 찍으면 문서의 「수량 터치 구간」과 「풋어웨이 터치 구간」을 얻는다. ⚠️ 두 구간은 겹칠 수 있고 겹침이 사실(교차 작업·동시 다인) — **합산 금지, 병기.** Receiving 표의 "Avg receive − holds"(현재 = 도크→완료, 풋어웨이 포함 — 부제에 명시 완료)를 열 2개로 가를지, 개명할지는 이때 함께 판단.
  3. **last_received_at 진실화** — 전송 시각이 아니라 터치 시각(진입점 3곳 processScan/manualSet/manualAdjust 에서 라인 객체에 캡처 → patchFor 가 실어 보냄). 기간 귀속(:3818)·작업자 표시(recvWorkers)가 정확해진다.
  ⚠️⚠️ **검증된 저장 경로 변경(수량 저장이 걸린다) — 단독 단계로 다룬다.** 동작 동일성 증명(2026-08-25 조사에서 확정한 절차): ① 변경 면적 diff — `patchFor` 호출자는 `writeLine` 하나뿐(grep 전수)·진입점 대입 3~5줄 ② 페이로드 키 집합 — **신규 컬럼은 마이그레이션 db push(Caleb 직접)가 프론트보다 먼저**(규칙 23 — 어기면 모든 qty UPDATE 가 400 = **수량 저장 전면 실패**, order_date 전례) ③ 시나리오 7건: 온라인 스캔 · offline→복구 flush · 같은 라인 재스캔 추월 · manualSet 충돌 분기 · off-PO · Place all · Hold→재개→완료.
  신뢰 시점 = 배포 익일 00:00 UTC 상계 + 첫 행 실측(verification_method 전례). **백필 금지** — 이른 터치 시각은 이미 덮여 소멸(비가역, vm 오염과 같은 성질).
...
### 동시 작업 원자화
- ~~⏸ **같은 사람 · 다중 기기(탭) 차단 — 설계 확정·구현·격자 검증까지 끝나고 되돌림 (2026-09-10 · 재개 조건 = 원장 `inv_layer` 마이그레이션 13건이 프로덕션에 먼저 올라가야 한다).**~~ → ✅ **2026-09-10 밤 재적용·배포·현장 검증 완료** (커밋 `23df53a` · 마이그레이션 `20260910230704` · 정본 규칙 28 「같은 사람 · 다른 기기」 소절 · 보고서 `docs/sessions/2026-09-10-multi-device-guard/v5_7-reapply-implementation.md`). [실측 2026-09-10 · Caleb] 프로덕션 pg_proc 4줄 전부 `p_session_id text` — **오버로드 잔존 0** · `session_id` 컬럼 3표 확인 · ⭐ **현장 검증 3종 통과**: ① 다른 탭이 열면 얼어붙음("another device that is signed in as you" 모달 실물) ② Reload 로 되찾으면 저쪽이 얼어붙음 — 대칭 성립 ③ 탭 하나만 열고 Ctrl+Shift+R → 아무 일 없음(**오탐 없음 — sessionStorage 선택의 실증**). 남은 것(별도): ⬜ **하루 관찰** — 작업자가 예상 밖으로 얼어붙는지(other_device 프리즈는 응답만 있고 로그가 없다 — 청취로 확인) · ⬜ **리시빙 2단계**(`wms_receipt_sessions (receipt_id, worker)` — 여러 사람 허용·같은 사람은 하나) · ⬜ 리시빙 같은 라인 CAS(델타 재적용) · ⬜ LIVE NOW 같은 사람 중복 표시(×2 배지 vs 화면 수). 경위(원문 유지): 같은 사람 두 기기는 `checkOwner`(`who===me.name`)·완료/Hold RPC CAS 를 전부 통과해 완료 스냅샷이 다른 기기의 픽을 0 으로 덮는다([실측] 실해 0건 — `picked_at` 있고 `verification_method` null 인 라인 0). RPC 4개 시그니처가 바뀌어 DB→HTML 순서가 강제되는데 `db push --linked` 가 원장 13건을 함께 올려 배포할 수 없었다. ⚠️ 정본·재개 절차·확정 설계(sessionStorage UUID · 작업 단위 잠금 · 새 기기가 이김) = `docs/sessions/2026-09-10-same-user-multi-device-guard-reverted.md` + 보고서 3건 `docs/sessions/2026-09-10-multi-device-guard/v5_{1,3,4}-*.md`(v5_4 에 RPC 4개 diff 실물 — 재개 시 그대로 재적용). 다시 조사하지 말 것.
...
- ✅ **bin 백필 스위프 작동 검증 — 닫힘 (2026-09-01 · 배포 `7c148e0` · 규칙 12 항목·세션 문서가 정본)**: [실측 2026-09-01] `runWmsMasterSync` 수동 실행으로 재고가 반영되자 **다음 폴링 회차(2분)에 `swept 0 → 8` · `candidates 8 → 1`** — ⭐ 「`no_bin` flag + 채워진 `bin_location` 공존 = 스위프의 서명」이 실물로 확인됐다. 남은 1줄은 `SKL00882`(sticky 에 기록 자체가 없는 SKU = **설계상 못 잡는 부류**, 결함 아님). ⚠️⚠️ **검증이 8월 31일에 안 됐던 이유가 별개 장애였다** — 후보를 채우려면 재고 스냅샷이 갱신돼야 하는데 `⟳ Stock` 체인이 12일 이상 죽어 있었다(규칙 6 「⟳ Stock 온디맨드 체인」 절 · `docs/sessions/2026-09-01-refresh-chain-dead.md`). 📌 **미검증 항목이 다른 장애의 증상일 수 있다 — 「기회가 없었다」로 넘기기 전에 왜 기회가 없었는지 물을 것.** ⬜ 잔여(검증과 무관): `needs_review` 잔존이 매니저 화면에서 거슬리는지 관찰 후 판단(Caleb 확정 — 지금은 무접촉).
...
### 신규 기능
- ⚠️⚠️ 🆕 **`⟳ Stock` 실패 가시성 — Health 검사 (2026-09-01 실사고 · ⬜ 미착수 · 규칙 6 「⟳ Stock 온디맨드 체인」 절이 구조 정본)**: 체인이 **12일 이상 반쪽만 돌았는데 아무 화면에도 안 떴다**(①Cin7→BQ 성공 / ②BQ→Supabase 는 GAS 트리거 20/20 한도로 미실행). 실패는 그 순간 admin 탭을 보던 사람에게만 보이고, **「BQ 만 최신」이라는 상태 자체가 표시되지 않는다.**
  - **처방**: `wms_health_check()` 에 검사 추가 — 최근 `wms_refresh_requests` 가 `status='error'` 이거나 `running` 으로 방치(예: 30분+). ⭐ **선례가 그대로 있다 — `image_sync_stale`**(2026-08-14 CSV 7주 사고의 처방 · 「상품 이미지 파이프라인」 절). 그때 문장이 오늘 그대로 적용된다: *「뿌리는 CSV 가 묵은 것이 아니라 **7주 묵은 것을 아무도 몰랐다**는 것 — 처방의 중심이 자동화만이 아니라 **감시**인 이유」*. 컬럼은 baseline 에 있다(`requested_by·requested_at·started_at·finished_at·status·note`).
  - ⚠️ **2026-09-01 에 트리거를 18개로 줄여 임시 해소했으나 여유가 2개뿐 — 재발한다.** 감시가 없으면 **다음 재발도 12일짜리**가 된다(트리거 개수는 GAS 런타임 상태라 코드로 확인 불가 — Caleb 실측).
  - 📌 **같은 부류의 두 번째다**(08-14 CSV · 09-01 ⟳ Stock) — 세 번째가 나오면 앵커 신설 검토(앵커 절 09-01 항목).
...
- ~~**picked_at 살리기 — 개별 SKU 스캔 시각 (2026-08-13)**: ⚠️ 비용이 크다~~ → ✅ **1단계 구현 (2026-08-21 — 기록 시작 · Stats 계산 교체는 2단계 별건)**. ⚠️⚠️ **종전 비용 ②("스캔마다 서버 쓰기 = 왕복 급증")는 오판이었다 — 그 오판이 이 항목을 8일간 보류시켰다**: `saveLine` 이 **이미 스캔·스테퍼·수동입력마다 UPDATE 1개를 보내고 있었다**(picker:1129·1144·1154 — 기존 왕복을 계산에 안 넣었다). 필드 편승 = **왕복 +0**(배터리 무영향). 📌 "비용이 커서 보류" 를 적을 땐 **기존 경로의 비용을 먼저 세어 볼 것**.
  ...
  - ⚠️ 잔류: **태블릿 시계 오차**(클라이언트 시각 수용의 대가 — CCTV 대조 시 위험) · ~~**reaper `started_at=null` 은 안 고쳤다**(새 자 검증 후 판단 — 지금 고치면 "4.1분"이 "3시간"이라는 새 거짓이 될 뿐)~~ → ✅ 2026-08-24 종결(자동 Hold 가 reaper 흡수·폐지 — SO-15028-1 절 종결 항목 참조) · 스캔 0 완료 구간·첫 스캔 전 준비 시간은 안 잡힌다(**과소 방향의 자** — 2단계 화면 표기로 명시 예정).
