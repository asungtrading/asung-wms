...
- ⚠️ **`started_at` 덮어쓰기 — avg time 왜곡 (규칙 23·28·37 · 2026-08-11 조사, 수정은 사용자 결정 대기)** — **[확정 — 코드]** 스캔 이어받기(picker `openByScan`·packer 스캔 이어받기)와 **Hold→재개**(held 카드가 `startBatch`/packer 큐 클레임 재사용 — Hold RPC 는 보존하는데 재개가 덮어씀)가 `started_at=now` 로 리셋해 완료자 시간이 **과소** 기록되고 인계자 시간은 소멸한다. wave 멤버는 `startWave` 일괄 기록으로 **과대**(전원 = wave 전체). `takeoverBatch`(방치 클레임)만 보존 — 같은 이어받기인데 설계가 갈라져 있다. **[정황 — 경로 미확정]** SO-14464-1(68라인/2분10초) 이 계기 — `picked_at` 전 행 null·롤백 로그 없음이라 판별 불가, **특정 경로로 단정 금지**. 수정 방향(참고): ①② 는 takeoverBatch 처럼 보존(1~2줄), ③ 은 wave 시간 배분 설계 결정 필요. 상세는 규칙 37 「avg time·min/line 의 신뢰 한계」.
