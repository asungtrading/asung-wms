...
### 신규 기능
- **Order Trace — CCTV 조회용 (2026-08-13 후보 · 규칙 9/37 계열)**: 손님 미수령 연락 → CCTV 조회 시 **어느 분할 배치에 어떤 물건이 있었고 누가 언제 작업했는지**로 시간·장소를 특정해 영상 확보를 빠르게. **매니저 한정**(`wms_staff.perms` 게이트 — 읽기 전용 DB 계정보다 이쪽: 급한 상황에 SQL 을 짤 수 없고, 필요한 답이 좁고 정해져 있다). 입력 = 오더 번호(+ SKU 필터) / 출력 = 배치별 SKU·수량·픽/팩 작업자·시작·완료 시각·창고. ⚠️ **시각은 토론토 로컬로 표시**(DB 는 UTC). ⚠️ **한계 = 배치 단위까지** — `picked_at` 이 죽은 컬럼(규칙 37 백로그 실측: 19,375행 전량 null)이라 개별 SKU 시각이 없다: 배치가 30분이면 CCTV 30분 구간. **빈도가 낮으니 "실제로 30분이 부담인지" 실측 후** 아래 picked_at 살리기를 판단.
...
### 정리 필요 (데이터)
- **SO-13893 어중간 상태 (2026-08-12 발견)** — 07-30 finalize(direct) 후 Undo Pack 오판 실행으로 status='picking'+finalized_at 잔존 2주. **Cin7 정상 출고 확인됨 → `closed` 로 정합성 회복이 방향**(세션 문서 11장 — 위 「Rollback 이 Finalize 단계를 모델링하지 않는다」 참조).
