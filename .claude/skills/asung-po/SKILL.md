...
```
① Settings (8축)  ✅ 09-11 — 7축(표 여덟 ref_) · 사용자는 `ims_staff`(§10-h) · 정본 §3-a
② 공급처           ✅ 09-12(§3-b 설계 · §3-c 적재) — supplier / _address / _contact / _discount
...
③ 제품             ✅ 09-14(§3-d 설계 · §3-e 결과 · 테스트 DB) — product_family / product / product_barcode / product_bom
                   · 범위 §3-d(Type=Stock − 대체UPC + 자재 1) · ⭐ pack_factor 정본 = BOM Quantity(§4) · 카운터 다섯 §3-e
④ 제품↔공급처      ✅ 09-14(§3-g · 테스트 DB) — product_supplier 1표 · 활성 12,721줄 · ⭐ 충돌 키 cin7_id
                   · supplier 257(비활성 31 포함) · is_default 는 우리 칸(§4) · 카운터 여섯 §3-g
⑤ PO 본체          ✅ 09-16 — 설계 **§11** · 표 **§13**(⚠️ 테스트 DB 한정) · 표 열하나 po·po_line·po_discount·po_receipt_line · po_invoice(doc_kind 로 크레딧 포함)·_line·_discount · po_charge·_alloc · po_payment·_alloc
                   · 읽기 뷰 po_list + RPC po_detail(⭐ **계산 규칙의 정본** — 화면에서 다시 짜지 마라) · 쓰기 RPC po_create · po_lines_paste(미리 보기 필수) · po_line_update/delete(막는 둘)
                   · 화면 po.html · ⭐ 확정은 잠금이 아니다(§11-b) · 🔄 다음 §13-f(넓은 목록 · 국면 다섯 · 머리 칸 · 만들기 차수)
화면               ✅ 09-16 — `ims.asung.ca`(레포 `asung-ims` · ⚠️ 공개) · 읽기 넷 + **쓰기 셋**(staff · suppliers is_purchasable · po.html 발주 만들기·편집) · 규칙 **§10-j** · ⬜ 채울 칸 **§10-k**
...
- ⚠️ 마스터 17표와 PO 표 열하나는 **테스트 DB(Asung-IMS)에만 있다** — `--db-url "$(cat ~/.asung-testdb-url)"` 가 보이면 테스트 · 없으면 운영.
...
| `product_supplier` | `(product_id, supplier_id)` · ⭐ **충돌 키는 `cin7_id`**(ProductSupplierID · manual 승격의 열쇠) | 관계 표 · manual 줄 승격(§3-g) · ⚠️ `is_default` 부분 유니크 금지(카운터) · 단가 `numeric(18,7)` |
...
```
- 검증 관례: `supabase start → db reset → information_schema 실물 → 실동작 → check-caps.sh → supabase stop`.
...
| 재적재를 표마다 다르게 · 「source=cin7 지우고 다시」 | 값(valid_from·note·id)이 사라진다 · upsert 는 「없어진 것」을 모른다 | ⭐ **넷이 한 규칙** — upsert + 안 들어온 cin7 행은 `is_active=false` · 정본 §3-f |
...
| 「목록에 없으면 상세를 비운다」 · 진입·이동 때 검색어를 안 바꾼다 | 언제 비는지 알 수 없다 · 고른 것에 파란 표시가 없다 | **사람이 바꾼** 검색·필터로만 비운다 · 페이지는 둔다 · 진입·이동은 SKU 를 검색칸에 · §10-j 3-d |
