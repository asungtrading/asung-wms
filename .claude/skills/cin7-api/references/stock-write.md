...
- **bin GUID 조회**: `GET /ref/location` → **최상위 창고 행(`ParentID` 없음)의 `Bins[]` 배열**.
  ...
  bin 이름이 아니다**(바코드류) — 그 폴백은 죽은 경로였다.~~ → ⚠️⚠️ **[정정 2026-09-11] child 행의 `Name` 은
  bin 이름이 맞다**(전량 2,678행 실측 · 2,675/2,675 일치). 그 폴백이 죽었던 이유는 이름이 아니라 **`Limit 500` 잘림**
  (에드먼튼 child 0행)이었다 — 그래서 `Bins[]` 경로가 여전히 정답이다. 상세는 아래 5절.
...
- ~~⚠️ **child-location 행의 `Name` 은 bin 이름이 아니다** — 예 `"071164313169"` 같은 바코드류다. 이름 매칭이 애초에 성립하지 않았다.~~
  → ⚠️⚠️ **[정정 2026-09-11] 틀린 기록이었다.** GAS 프로브로 `ref/location` 을 `Limit` 페이지네이션으로 **전량 2,678행** 수집해 확인:
  ```
  창고 Bins[] 합계              2,675 (중복 제거 2,675)
  하위 행(ParentID 있음) 총수    2,675
  하위 행 Name 이 Bins[] 에 있음  2,675
  없음                              0
  하위 Name 숫자만                  0   ·   글자 포함 2,675   ·   20자 이상 0
  ```
  ⇒ **bin 이름은 하위 행 `Name` 에서 그대로 얻을 수 있다. 바코드는 섞여 있지 않다.** `Bins[]` 원소 `{ID, Name, IsDeprecated, IsStaging}` 의
  `ID` 는 하위 행의 `ID` 와 같다 — 어느 쪽에서 얻어도 같은 bin GUID 다.
  ⚠️ 2026-07-28 기록의 `"071164313169"` 가 **어느 응답의 무엇을 본 것인지는 모른다(구간·기준 불명)** — 이유를 지어내지 않는다.
  실사고 TR-02935 의 원인은 위 항목의 **`Limit 500` 잘림**(에드먼튼 child 0행)으로 충분히 설명되고, `Bins[]` 만 쓰라는 처방은 그대로 유효하다.
  전량 필드·창고 GUID 는 `references/ref-endpoints.md`.
