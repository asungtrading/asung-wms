...
3. **Rate Limit**: 루프에서 `Utilities.sleep(300)` 추가 권장. ⚠️ **sleep 만으로는 부족하다** — 같은 키를 쓰는 다른 호출과 겹치면 429 가 그래도 온다. 429 처리 원칙은 아래 11번. ⚠️ **정정(2026-08-18): API 키가 WMS 용과 GAS 용으로 분리돼 있다** — GAS 프로브는 WMS 키 예산과 무관하다. 단 **한도가 키 단위인지 계정 단위인지는 미확인**(429 백오프 원칙 자체는 유지).
